...
/* Function prototypes */
static void LED_Driver_Gear_DmaInit(void) ;
static void LED_Driver_Gear_DmaEnable(void) ;
static void LED_Driver_Gear_DmaDisable(void) ;

/* Used to hold the common line output sequence */
#if(LED_Driver_Gear_COM_DRIVE == LED_Driver_Gear_HIGH)
    static uint8 LED_Driver_Gear_comBuffer[16] = {0x00u, 0x01u, 0x00u, 0x02u, 0x00u, 0x04u, 0x00u, 0x08u,
                                                    0x00u, 0x10u, 0x00u, 0x20u, 0x00u, 0x40u, 0x00u, 0x80u};
#else
    static uint8 LED_Driver_Gear_comBuffer[16] = {0xFFu, 0xFEu, 0xFFu, 0xFDu, 0xFFu, 0xFBu, 0xFFu, 0xF7u,
                                                    0xFFu, 0xEFu, 0xFFu, 0xDFu, 0xFFu, 0xBFu, 0xFFu, 0x7Fu};
#endif /* (LED_Driver_Gear_COM_DRIVE == LED_Driver_Gear_HIGH) */

/* Buffers used to hold the display(segment) values to be transferred to the control registers */
uint8 LED_Driver_Gear_segLBuffer[LED_Driver_Gear_NUM_COM];
#if (LED_Driver_Gear_NUM_SEG > 8u) 
    uint8 LED_Driver_Gear_segMBuffer[LED_Driver_Gear_NUM_COM];
#endif /* (LED_Driver_Gear_NUM_SEG > 8u) */
#if (LED_Driver_Gear_NUM_SEG > 16u) 
    uint8 LED_Driver_Gear_segHBuffer[LED_Driver_Gear_NUM_COM];
#endif /* (LED_Driver_Gear_NUM_SEG > 16u) */

#if (LED_Driver_Gear_BRIGHTNESS_EN == LED_Driver_Gear_ENABLED)
    /* The brightness register used to hold the brightness levels for each common */
    static uint8 LED_Driver_Gear_brightnessReg[LED_Driver_Gear_NUM_COM];
#endif /* (LED_Driver_Gear_BRIGHTNESS_EN == LED_Driver_Gear_ENABLED) */

/* DMA channel and TD declaractions */
static uint8 LED_Driver_Gear_dmaComChannel;
static uint8 LED_Driver_Gear_dmaComTd[LED_Driver_Gear_TWO_NUM_COM];

static uint8 LED_Driver_Gear_dmaSegChannel;
static uint8 LED_Driver_Gear_dmaSegLTd[LED_Driver_Gear_NUM_COM];

#if (LED_Driver_Gear_BRIGHTNESS_EN == LED_Driver_Gear_ENABLED)
    static uint8 LED_Driver_Gear_dmaBCChannel;
    static uint8 LED_Driver_Gear_dmaBCTd;
#endif /* (LED_Driver_Gear_BRIGHTNESS_EN == LED_Driver_Gear_ENABLED) */

uint8 LED_Driver_Gear_initVar = 0u;
uint8 LED_Driver_Gear_enableVar = 0u;


/*******************************************************************************
* Function Name: LED_Driver_Gear_DmaInit
********************************************************************************
*
* Summary: 
...
static void LED_Driver_Gear_DmaInit(void) 
{
    ...
    #if (LED_Driver_Gear_NUM_SEG > 8u)
        static uint8 LED_Driver_Gear_dmaSegMTd[LED_Driver_Gear_NUM_COM];
    #endif /* (LED_Driver_Gear_NUM_SEG > 8u) */
    #if (LED_Driver_Gear_NUM_SEG > 16u)
        static uint8 LED_Driver_Gear_dmaSegHTd[LED_Driver_Gear_NUM_COM];
    #endif /* (LED_Driver_Gear_NUM_SEG > 16u) */

    /************************* DMA configuration for the Commons *************************/
    /* Common DMA channel */
    LED_Driver_Gear_dmaComChannel = LED_Driver_Gear_DMA_Com_DmaInitialize(1u, 1u, 
                                        HI16(LED_Driver_Gear_SRAM_BASE), HI16(LED_Driver_Gear_CONTROL_REG_BASE));

    /* Check if the channel is valid */                                                                        
    if(LED_Driver_Gear_dmaComChannel != DMA_INVALID_CHANNEL)
    {
        /* Allocate the common TDs */
        for(i = 0u; i < LED_Driver_Gear_TWO_NUM_COM; i++)
        {
            LED_Driver_Gear_dmaComTd[i] = CyDmaTdAllocate();
        }
        
        /* Set configuration for the Common TDs */
        for(i = 0u;  i < (LED_Driver_Gear_TWO_NUM_COM - 1u); i++)
        {
            /* Separate configurations for odd and even TDs */
            if((uint8)(i & 1u) == 0u)
            {
                /* Set configuration for the Common TDs */
                (void) CyDmaTdSetConfiguration(LED_Driver_Gear_dmaComTd[i], 1u, 
                                                LED_Driver_Gear_dmaComTd[i+1u],
                                                TD_INC_SRC_ADR | LED_Driver_Gear_DMA_Com__TD_TERMOUT_EN);
            }
            else
            {
                /* Set configuration for the Common TDs */
                (void) CyDmaTdSetConfiguration(LED_Driver_Gear_dmaComTd[i], 1u,
                                                LED_Driver_Gear_dmaComTd[i+1u], TD_INC_SRC_ADR);
            }
        }
        /* Set the wraparound Common TD configuration */
        (void) CyDmaTdSetConfiguration(LED_Driver_Gear_dmaComTd[LED_Driver_Gear_TWO_NUM_COM - 1u], 
                                        1u, LED_Driver_Gear_dmaComTd[0u], TD_INC_SRC_ADR);
        
        /* Set the lower addresses of the common TDs */
        for(i = 0u; i < LED_Driver_Gear_TWO_NUM_COM; i++)
        {
            (void) CyDmaTdSetAddress(LED_Driver_Gear_dmaComTd[i],
                                        LO16((uint32) &LED_Driver_Gear_comBuffer[i]),
                                        LO16((uint32) LED_Driver_Gear_COM_CONTROL_PTR));
        }    
    }
    ...
    LED_Driver_Gear_dmaSegChannel = LED_Driver_Gear_DMA_Seg_DmaInitialize(1u, 1u, 
                                        HI16(LED_Driver_Gear_SRAM_BASE), HI16(LED_Driver_Gear_CONTROL_REG_BASE));
    
    /* Check if the segment channel is valid */
    if(LED_Driver_Gear_dmaSegChannel != DMA_INVALID_CHANNEL)
    {
        /* Allocate separate Tds for segments to implement the chain */
        for(i = 0u; i < LED_Driver_Gear_NUM_COM; i++)
        {
            LED_Driver_Gear_dmaSegLTd[i] = CyDmaTdAllocate();
            #if (LED_Driver_Gear_NUM_SEG > 8u)
                LED_Driver_Gear_dmaSegMTd[i] = CyDmaTdAllocate();
            #endif /* (LED_Driver_Gear_NUM_SEG > 8u) */
            #if (LED_Driver_Gear_NUM_SEG > 16u)
                LED_Driver_Gear_dmaSegHTd[i] = CyDmaTdAllocate();
            #endif /* (LED_Driver_Gear_NUM_SEG > 16u) */
        }
        
        /* Set the configuration for the Segment TDs */
        for(i = 0u; i < (LED_Driver_Gear_NUM_COM - 1u); i++)
        {
            #if (LED_Driver_Gear_NUM_SEG <= 8u)
                (void) CyDmaTdSetConfiguration(LED_Driver_Gear_dmaSegLTd[i], 1u, LED_Driver_Gear_dmaSegLTd[i+1u],
                                                TD_INC_SRC_ADR | LED_Driver_Gear_DMA_Seg__TD_TERMOUT_EN);
            #else    
                (void) CyDmaTdSetConfiguration(LED_Driver_Gear_dmaSegLTd[i], 1u, LED_Driver_Gear_dmaSegMTd[i],
                                                TD_INC_SRC_ADR | TD_AUTO_EXEC_NEXT);

                #if (LED_Driver_Gear_NUM_SEG <= 16u)
                    (void) CyDmaTdSetConfiguration(LED_Driver_Gear_dmaSegMTd[i], 1u, LED_Driver_Gear_dmaSegLTd[i+1u],
                                                    TD_INC_SRC_ADR | LED_Driver_Gear_DMA_Seg__TD_TERMOUT_EN);
                #else    
                    (void) CyDmaTdSetConfiguration(LED_Driver_Gear_dmaSegMTd[i], 1u, LED_Driver_Gear_dmaSegHTd[i],
                                                    TD_INC_SRC_ADR | TD_AUTO_EXEC_NEXT);
                    (void) CyDmaTdSetConfiguration(LED_Driver_Gear_dmaSegHTd[i], 1u, LED_Driver_Gear_dmaSegLTd[i+1u],
                                                    TD_INC_SRC_ADR | LED_Driver_Gear_DMA_Seg__TD_TERMOUT_EN);
                #endif /* LED_Driver_Gear_NUM_SEG <= 16u */
            #endif /* LED_Driver_Gear_NUM_SEG <= 8u */
        }
        ...
        #if (LED_Driver_Gear_NUM_SEG <= 8u)
            (void) CyDmaTdSetConfiguration(LED_Driver_Gear_dmaSegLTd[LED_Driver_Gear_NUM_COM - 1u],
                                            1u, LED_Driver_Gear_dmaSegLTd[0u], 
                                            TD_INC_SRC_ADR | LED_Driver_Gear_DMA_Seg__TD_TERMOUT_EN);
        #else    
            (void) CyDmaTdSetConfiguration(LED_Driver_Gear_dmaSegLTd[LED_Driver_Gear_NUM_COM - 1u],
                                            1u, LED_Driver_Gear_dmaSegMTd[LED_Driver_Gear_NUM_COM - 1u],
                                            TD_INC_SRC_ADR | TD_AUTO_EXEC_NEXT);
            #if (LED_Driver_Gear_NUM_SEG <= 16u)
                (void) CyDmaTdSetConfiguration(LED_Driver_Gear_dmaSegMTd[LED_Driver_Gear_NUM_COM - 1u],
                                                1u, LED_Driver_Gear_dmaSegLTd[0u], 
                                                TD_INC_SRC_ADR | LED_Driver_Gear_DMA_Seg__TD_TERMOUT_EN);
            #else    
                (void) CyDmaTdSetConfiguration(LED_Driver_Gear_dmaSegMTd[LED_Driver_Gear_NUM_COM - 1u],
                                                1u, LED_Driver_Gear_dmaSegHTd[LED_Driver_Gear_NUM_COM - 1u], 
                                                TD_INC_SRC_ADR | TD_AUTO_EXEC_NEXT);
                (void) CyDmaTdSetConfiguration(LED_Driver_Gear_dmaSegHTd[LED_Driver_Gear_NUM_COM - 1u],
                                                1u, LED_Driver_Gear_dmaSegLTd[0u], 
                                                TD_INC_SRC_ADR | LED_Driver_Gear_DMA_Seg__TD_TERMOUT_EN);
            #endif /* (LED_Driver_Gear_NUM_SEG <= 16u) */
        #endif /* (LED_Driver_Gear_NUM_SEG <= 8u) */
        ...
        for(i = 0u; i < LED_Driver_Gear_NUM_COM; i++)
        {
            (void) CyDmaTdSetAddress(LED_Driver_Gear_dmaSegLTd[i],
                                            LO16((uint32) &LED_Driver_Gear_segLBuffer[i]),
                                            LO16((uint32) LED_Driver_Gear_SEG_L_CONTROL_PTR));
            #if (LED_Driver_Gear_NUM_SEG > 8u)
                (void) CyDmaTdSetAddress(LED_Driver_Gear_dmaSegMTd[i],
                                            LO16((uint32) &LED_Driver_Gear_segMBuffer[i]),
                                            LO16((uint32) LED_Driver_Gear_SEG_M_CONTROL_PTR));
            #endif /* (LED_Driver_Gear_NUM_SEG > 8u) */
            #if (LED_Driver_Gear_NUM_SEG > 16u)
                (void) CyDmaTdSetAddress(LED_Driver_Gear_dmaSegHTd[i], 
                                            LO16((uint32) &LED_Driver_Gear_segHBuffer[i]),
                                            LO16((uint32) LED_Driver_Gear_SEG_H_CONTROL_PTR));
            #endif /* (LED_Driver_Gear_NUM_SEG > 16u) */
        }
    }
    ...
    /******************** DMA configuration for Brightness control *********************/
        ...
        LED_Driver_Gear_dmaBCChannel = LED_Driver_Gear_DMA_BC_DmaInitialize(1u, 1u, 
                                        HI16(LED_Driver_Gear_SRAM_BASE), HI16(LED_Driver_Gear_CONTROL_REG_BASE));
            
        /* Check if the channels are valid */
        if(LED_Driver_Gear_dmaBCChannel != DMA_INVALID_CHANNEL)
        {
            LED_Driver_Gear_dmaBCTd = CyDmaTdAllocate();
            
            /* Set the TD configuration for the brightness DMA */
            (void) CyDmaTdSetConfiguration(LED_Driver_Gear_dmaBCTd, LED_Driver_Gear_NUM_COM,
                                        LED_Driver_Gear_dmaBCTd, TD_INC_SRC_ADR);
            
            /* Lower address for the brightness Td */
            (void) CyDmaTdSetAddress(LED_Driver_Gear_dmaBCTd, LO16((uint32) &LED_Driver_Gear_brightnessReg[0u]),
                                                               LO16((uint32)LED_Driver_Gear_COMPARE_PTR));
        }
    #endif /* (LED_Driver_Gear_BRIGHTNESS_EN == LED_Driver_Gear_ENABLED) */
}
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_DmaEnable
********************************************************************************
*
* Summary: 
...
static void LED_Driver_Gear_DmaEnable(void) 
{
    ...
    (void) CyDmaChSetInitialTd(LED_Driver_Gear_dmaComChannel, LED_Driver_Gear_dmaComTd[0u]);
    (void) CyDmaChSetInitialTd(LED_Driver_Gear_dmaSegChannel, LED_Driver_Gear_dmaSegLTd[0u]);
    #if (LED_Driver_Gear_BRIGHTNESS_EN == LED_Driver_Gear_ENABLED)
        (void) CyDmaChSetInitialTd(LED_Driver_Gear_dmaBCChannel, LED_Driver_Gear_dmaBCTd);
    #endif /* (LED_Driver_Gear_BRIGHTNESS_EN == LED_Driver_Gear_ENABLED) */
    
    /* Enable all channels */
    (void) CyDmaChEnable(LED_Driver_Gear_dmaComChannel, 1u);
    (void) CyDmaChEnable(LED_Driver_Gear_dmaSegChannel, 1u);
    #if (LED_Driver_Gear_BRIGHTNESS_EN == LED_Driver_Gear_ENABLED)
        (void) CyDmaChEnable(LED_Driver_Gear_dmaBCChannel, 1u);
    #endif /* (LED_Driver_Gear_BRIGHTNESS_EN == LED_Driver_Gear_ENABLED) */
}


/*******************************************************************************
* Function Name: LED_Driver_Gear_DmaDisable
********************************************************************************
*
* Summary: 
...
static void LED_Driver_Gear_DmaDisable(void) 
{
    uint8 i;
    uint8 channel;
    uint8 dmaChannelList[LED_Driver_Gear_CHANNEL_LIMIT];
    
    dmaChannelList[0u] = LED_Driver_Gear_dmaComChannel;
    dmaChannelList[1u] = LED_Driver_Gear_dmaSegChannel;
    #if (LED_Driver_Gear_BRIGHTNESS_EN == LED_Driver_Gear_ENABLED)
        dmaChannelList[2u] = LED_Driver_Gear_dmaBCChannel;
    #endif /* (LED_Driver_Gear_BRIGHTNESS_EN == LED_Driver_Gear_ENABLED) */

    for(i = 0u; i < LED_Driver_Gear_CHANNEL_LIMIT; i++)
    {
        channel = dmaChannelList[i];
        (void) CyDmaChDisable(channel);
        
        LED_Driver_Gear_Com_Driver_Write(LED_Driver_Gear_CLEAR);
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_Init
********************************************************************************
*
* Summary: 
...
{
    static uint8 LED_Driver_Gear_dmaInitDone = 0u;
    #if (LED_Driver_Gear_BRIGHTNESS_EN == LED_Driver_Gear_ENABLED)
        uint8 i;
    #endif /* (LED_Driver_Gear_BRIGHTNESS_EN == LED_Driver_Gear_ENABLED) */
    
    /* Start fresh by clearing the display */
    LED_Driver_Gear_ClearDisplayAll();
    
    /* Initialize the DMAs */
    if(0u == LED_Driver_Gear_dmaInitDone)
    {
        LED_Driver_Gear_DmaInit();
        LED_Driver_Gear_dmaInitDone = 1u;
    }
    ...
    /* Set the brightness to full */
        ...
        for(i = 0u; i<(LED_Driver_Gear_NUM_COM); i++)
        {
            LED_Driver_Gear_brightnessReg[i] = LED_Driver_Gear_INIT_BRIGHT;
        }
    #endif /* (LED_Driver_Gear_BRIGHTNESS_EN == LED_Driver_Gear_ENABLED) */
}
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_Enable
********************************************************************************
*
* Summary: 
...
{
    /* Enable the DMAs */
    LED_Driver_Gear_DmaEnable();
 
    #if (LED_Driver_Gear_BRIGHTNESS_EN == LED_Driver_Gear_ENABLED)
       /* Initialize to zero */
       LED_Driver_Gear_COUNTER_REG = 0x00u;
       LED_Driver_Gear_COMPARE_REG = 0x00u;
       
        /* Enable the PWM for brightness control */
       LED_Driver_Gear_CONTROL_REG = LED_Driver_Gear_CTRL_ENABLE;
    #endif /* (LED_Driver_Gear_BRIGHTNESS_EN == LED_Driver_Gear_ENABLED) */
    
    LED_Driver_Gear_enableVar = LED_Driver_Gear_CTRL_ENABLE;
}


/*******************************************************************************
* Function Name: LED_Driver_Gear_Start
********************************************************************************
*
* Summary:
...
{
    if(0u == LED_Driver_Gear_initVar)
    {
        LED_Driver_Gear_Init();
        LED_Driver_Gear_initVar = 1u;
    }
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_Stop
********************************************************************************
*
* Summary:
...
void LED_Driver_Gear_Stop(void) 
{
    #if (LED_Driver_Gear_BRIGHTNESS_EN == LED_Driver_Gear_ENABLED)
        /* Disable the PWM */
        LED_Driver_Gear_CONTROL_REG = LED_Driver_Gear_CTRL_DISABLE;
    #endif /* (LED_Driver_Gear_BRIGHTNESS_EN == LED_Driver_Gear_ENABLED) */
    
    LED_Driver_Gear_DmaDisable();
    LED_Driver_Gear_ClearDisplayAll();
    
    LED_Driver_Gear_enableVar  = LED_Driver_Gear_CTRL_DISABLE;
}


/*******************************************************************************
* Function Name: LED_Driver_Gear_SetDisplayRAM
********************************************************************************
*
* Summary:
...
*  RAM directly by writing to LED_Driver_Gear_segLBuffer,
*  LED_Driver_Gear_segMBuffer, LED_Driver_Gear_segHBuffer.
*
* Parameters:
...
void LED_Driver_Gear_SetDisplayRAM(uint8 value, uint8 position) 
{
    ...
    if(position < LED_Driver_Gear_NUM_COM)
    {
        relPosition = position;
        #if(LED_Driver_Gear_SEG_DRIVE == LED_Driver_Gear_HIGH)
            LED_Driver_Gear_segLBuffer[relPosition] = value;
        #else
            LED_Driver_Gear_segLBuffer[relPosition] = ((uint8) ~value);
        #endif /* LED_Driver_Gear_SEG_DRIVE == LED_Driver_Gear_HIGH */
    }    
    else if (position < LED_Driver_Gear_TWO_NUM_COM)
    {
        #if (LED_Driver_Gear_NUM_SEG > 8u)
            relPosition = position - LED_Driver_Gear_NUM_COM;
            #if(LED_Driver_Gear_SEG_DRIVE == LED_Driver_Gear_HIGH)
                LED_Driver_Gear_segMBuffer[relPosition] = value;
            #else
                LED_Driver_Gear_segMBuffer[relPosition] = ((uint8) ~value);
            #endif /* LED_Driver_Gear_SEG_DRIVE == LED_Driver_Gear_HIGH */
        #endif /* (LED_Driver_Gear_NUM_SEG > 8u) */
    }
    else
    {
        #if (LED_Driver_Gear_NUM_SEG > 16u)
            relPosition = position - LED_Driver_Gear_TWO_NUM_COM;
            if(relPosition < LED_Driver_Gear_NUM_COM)
            {
                #if(LED_Driver_Gear_SEG_DRIVE == LED_Driver_Gear_HIGH)
                    LED_Driver_Gear_segHBuffer[relPosition] = value;
                #else
                    LED_Driver_Gear_segMBuffer[relPosition] = ((uint8) ~value);
                #endif /* LED_Driver_Gear_SEG_DRIVE == LED_Driver_Gear_HIGH */
            }
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_SetRC
********************************************************************************
*
* Summary:
...
void LED_Driver_Gear_SetRC(uint8 row, uint8 column) 
{
    ...
    if(column < LED_Driver_Gear_NUM_COM)
    {
        if(row < 8u)
        {
            rowSeg = row;
            #if(LED_Driver_Gear_SEG_DRIVE == LED_Driver_Gear_HIGH)
                LED_Driver_Gear_segLBuffer[column] |= ((uint8)((uint8)0x01u << rowSeg));
            #else
                LED_Driver_Gear_segLBuffer[column] &= ((uint8)~((uint8)((uint8)0x01u << rowSeg)));
            #endif /* LED_Driver_Gear_SEG_DRIVE == LED_Driver_Gear_HIGH */
        }
        else if (row < 16u)
        {
                ...
                rowSeg = row - 8u;
                #if(LED_Driver_Gear_SEG_DRIVE == LED_Driver_Gear_HIGH)
                    LED_Driver_Gear_segMBuffer[column] |= ((uint8)((uint8)0x01u << rowSeg));
                #else
                    LED_Driver_Gear_segMBuffer[column] &= ((uint8)~((uint8)((uint8)0x01u << rowSeg)));
                #endif /* LED_Driver_Gear_SEG_DRIVE == LED_Driver_Gear_HIGH */
            #endif /* (LED_Driver_Gear_NUM_SEG > 8u) */
        }
        else
        {
                ...
                rowSeg = row - 16u;
                #if(LED_Driver_Gear_SEG_DRIVE == LED_Driver_Gear_HIGH)
                    LED_Driver_Gear_segHBuffer[column] |= ((uint8)((uint8)0x01u << rowSeg));
                #else
                    LED_Driver_Gear_segHBuffer[column] &= ((uint8)~((uint8)((uint8)0x01u << rowSeg)));
                #endif /* LED_Driver_Gear_SEG_DRIVE == LED_Driver_Gear_HIGH */
            #endif /* (LED_Driver_Gear_NUM_SEG > 16u) */
        }
    }
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_ClearRC
********************************************************************************
*
* Summary:
...
void LED_Driver_Gear_ClearRC(uint8 row, uint8 column) 
{
    ...
    if(column < LED_Driver_Gear_NUM_COM)
    {
        if(row < 8u)
        {
            ...
            #if(LED_Driver_Gear_SEG_DRIVE == LED_Driver_Gear_HIGH)
                LED_Driver_Gear_segLBuffer[column] &= ((uint8)(~(uint8)rowSelect));
            #else
                LED_Driver_Gear_segLBuffer[column] |= rowSelect;
            #endif /* LED_Driver_Gear_SEG_DRIVE == LED_Driver_Gear_HIGH */
        }    
        else if (row < 16u)
        {
                ...
                rowSelect = ((uint8)((uint8)0x01u << rowSeg));
                #if(LED_Driver_Gear_SEG_DRIVE == LED_Driver_Gear_HIGH)
                    LED_Driver_Gear_segMBuffer[column] &= ((uint8)(~(uint8)rowSelect));
                #else
                    LED_Driver_Gear_segMBuffer[column] |= rowSelect;
                #endif /* LED_Driver_Gear_SEG_DRIVE == LED_Driver_Gear_HIGH */
            #endif /* (LED_Driver_Gear_NUM_SEG > 8u) */
        }
        else
        {
                ...
                rowSelect = ((uint8)((uint8)0x01u << rowSeg));
                #if(LED_Driver_Gear_SEG_DRIVE == LED_Driver_Gear_HIGH)
                    LED_Driver_Gear_segHBuffer[column] &= ((uint8)(~(uint8)rowSelect));
                #else
                    LED_Driver_Gear_segHBuffer[column] |= rowSelect;
                #endif /* LED_Driver_Gear_SEG_DRIVE == LED_Driver_Gear_HIGH */
            #endif /* (LED_Driver_Gear_NUM_SEG > 16u) */
        }
    }
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_ToggleRC
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_GetRC
********************************************************************************
*
* Summary:
...
uint8 LED_Driver_Gear_GetRC(uint8 row, uint8 column) 
{
    ...
    if(column < LED_Driver_Gear_NUM_COM)
    {
        ...
        {
            rowSeg = row;
            rowSegSelect = (uint8)((uint8)LED_Driver_Gear_segLBuffer[column] >> rowSeg);
        }    
        else if (row < 16u)
        {
            #if (LED_Driver_Gear_NUM_SEG > 8u) 
                rowSeg = row - 8u;
                rowSegSelect = (uint8)((uint8)LED_Driver_Gear_segMBuffer[column] >> rowSeg);
            #endif /* (LED_Driver_Gear_NUM_SEG > 8u) */
        }
        else
        {
            #if (LED_Driver_Gear_NUM_SEG > 16u)
                rowSeg = row - 16u;
                rowSegSelect = (uint8)((uint8)LED_Driver_Gear_segHBuffer[column] >> rowSeg);
            #endif /* (LED_Driver_Gear_NUM_SEG > 16u) */
        }
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_ClearDisplay
********************************************************************************
*
* Summary:
...
{  
    ...
    {
        relPosition = position;
        LED_Driver_Gear_segLBuffer[relPosition] = LED_Driver_Gear_CLEAR;
    }    
    else if (position < LED_Driver_Gear_TWO_NUM_COM)
    {
        #if (LED_Driver_Gear_NUM_SEG > 8u)
            relPosition = position - LED_Driver_Gear_NUM_COM;
            LED_Driver_Gear_segMBuffer[relPosition] = LED_Driver_Gear_CLEAR;
        #endif /* (LED_Driver_Gear_NUM_SEG > 8u) */
    }
    else
    {
            ...
            if(relPosition < LED_Driver_Gear_NUM_COM)
            {
                LED_Driver_Gear_segHBuffer[relPosition] = LED_Driver_Gear_CLEAR;
            }
        #endif /* (LED_Driver_Gear_NUM_SEG > 16u) */
    }
}
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_ClearDisplayAll
********************************************************************************
*
* Summary:
...
void LED_Driver_Gear_ClearDisplayAll(void) 
{
    ...
    for(position = 0u; position < LED_Driver_Gear_NUM_COM; position++)
    {
        LED_Driver_Gear_segLBuffer[position] = LED_Driver_Gear_CLEAR; 
        
        #if (LED_Driver_Gear_NUM_SEG > 8u) 
            LED_Driver_Gear_segMBuffer[position] = LED_Driver_Gear_CLEAR;
        #endif /* (LED_Driver_Gear_NUM_SEG > 8u) */
        
        #if (LED_Driver_Gear_NUM_SEG > 16u)
            LED_Driver_Gear_segHBuffer[position] = LED_Driver_Gear_CLEAR;
        #endif /* (LED_Driver_Gear_NUM_SEG > 16u) */
    }
}
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_Write7SegNumberDec
********************************************************************************
*
* Summary:
...

{
    ...
    if(position < LED_Driver_Gear_NUM_COM)
    {
        choiceReg = LED_Driver_Gear_SEG_REG_LOW;
        relPosition = position;
    }
    else if (position < LED_Driver_Gear_TWO_NUM_COM)
    {
        choiceReg = LED_Driver_Gear_SEG_REG_MID;
        relPosition = position - LED_Driver_Gear_NUM_COM;
    }
    else
    {
        choiceReg = LED_Driver_Gear_SEG_REG_HIGH;
        relPosition = position - LED_Driver_Gear_TWO_NUM_COM;
    }
    
    /* Routine to extract the sign and to convert to unsigned */
    if(number >= LED_Driver_Gear_ZERO_NEG)
    {
        numberVal = (uint32)number;
        numberSign = LED_Driver_Gear_UNSIGNED;
    }
    else
    {
        numberVal = (uint32)(-number);
        numberSign = LED_Driver_Gear_SIGNED;
    }
    ...
    for(i = digits; i > 0u; i--)
    {
        ...
        bufArray[i-1u] = LED_Driver_Gear_EncodeNumber7Seg(selectVal);
        numberVal = numberVal / 10u;
    }
    
    /* Clean up the array and place sign if needed */
    if(alignment == LED_Driver_Gear_ZERO_PAD)
    {
        if(numberSign == LED_Driver_Gear_SIGNED)
        {
            /* Place sign on the MSB */
            bufArray[0u] = LED_Driver_Gear_EncodeChar7Seg('-');
        }
    }
    else
    {
        for(i = 0u; i < digits; i++)
        {
            if (bufArray[i] != LED_Driver_Gear_ZERO_7CHAR)
            {
                if(numberSign == LED_Driver_Gear_SIGNED)
                {
                    if(i == 0u)
                    {
                        /* Place sign on the MSB */
                        bufArray[i] = LED_Driver_Gear_EncodeChar7Seg('-');
                    }
                    else
                    {
                        /* Place sign before MSB */
                        bufArray[i-1u] = LED_Driver_Gear_EncodeChar7Seg('-');
                    }
                }
            ...
            else
            {
                if(zeroCount != (digits - 1u))
                {
                    ...
                    bufArray[i] = LED_Driver_Gear_CLEAR;
                    zeroCount++;
                }
            }    
        }
    ...
    if(alignment == LED_Driver_Gear_LEFT_ALIGN)
    {
        for(i = 0u; i < digits; i++)
        {
            if(bufArray[i] != LED_Driver_Gear_CLEAR)
            {
                if(relPosition < LED_Driver_Gear_NUM_COM)
                {
                    ...
                    switch(choiceReg)
                    {
                        case LED_Driver_Gear_SEG_REG_LOW :
                            LED_Driver_Gear_segLBuffer[relPosition] = bufArray[i];
                            break;
                        case LED_Driver_Gear_SEG_REG_MID :
                            #if (LED_Driver_Gear_NUM_SEG > 8u)
                                LED_Driver_Gear_segMBuffer[relPosition] = bufArray[i];
                            #endif /* (LED_Driver_Gear_NUM_SEG > 8u) */
                            break;
                        case LED_Driver_Gear_SEG_REG_HIGH :
                            #if (LED_Driver_Gear_NUM_SEG > 16u)
                                LED_Driver_Gear_segHBuffer[relPosition] = bufArray[i];
                            #endif /* (LED_Driver_Gear_NUM_SEG > 16u) */
                            break;
                        default :
                            LED_Driver_Gear_segLBuffer[relPosition] = bufArray[i];
    ...
    else
    {
        ...
        for(i = digits; i > 0u; i--)
        {
            if(((i + relPosition) - 1u) < LED_Driver_Gear_NUM_COM)
            {
                /* Decide which row to use */
                switch(choiceReg)
                {
                    case LED_Driver_Gear_SEG_REG_LOW :
                        LED_Driver_Gear_segLBuffer[(i + relPosition) - 1u] = bufArray[i-1u];
                        break;
                    case LED_Driver_Gear_SEG_REG_MID :
                        #if (LED_Driver_Gear_NUM_SEG > 8u)
                            LED_Driver_Gear_segMBuffer[(i + relPosition) - 1u] = bufArray[i-1u];
                        #endif /* (LED_Driver_Gear_NUM_SEG > 8u) */
                        break;
                    case LED_Driver_Gear_SEG_REG_HIGH :
                        #if (LED_Driver_Gear_NUM_SEG > 16u)
                            LED_Driver_Gear_segHBuffer[(i + relPosition) - 1u] = bufArray[i-1u];
                        #endif /* (LED_Driver_Gear_NUM_SEG > 16u) */
                        break;
                    default :
                        LED_Driver_Gear_segLBuffer[(i + relPosition) - 1u] = bufArray[i-1u];
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_Write7SegNumberHex
********************************************************************************
*
* Summary:
...

{
    ...
    for(i = digits; i > 0u; i--)
    {
        selectVal = (uint8)((uint8)numberVal & LED_Driver_Gear_NIBBLE_MASK);
        bufArray[i-1u] = LED_Driver_Gear_EncodeNumber7Seg(selectVal);
        numberVal = (uint32)((uint32)numberVal >> LED_Driver_Gear_SHIFT_NIBBLE);
    }
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_WriteString7Seg
********************************************************************************
*
* Summary:
...

{
    ...
    if(position < LED_Driver_Gear_NUM_COM)
    {
        ...
        for(relPosition = position; relPosition < LED_Driver_Gear_NUM_COM; relPosition++)
        {
            if(relPosition < sizeLimit)
            {
                LED_Driver_Gear_segLBuffer[relPosition] = LED_Driver_Gear_EncodeChar7Seg(character[i]);
                i++;
            }
        }
    }    
    ...
    {
        #if (LED_Driver_Gear_NUM_SEG > 8u) 
            sizeLimit = ((uint8)(strlen(character)) + position) - LED_Driver_Gear_NUM_COM;
            for(relPosition = position - LED_Driver_Gear_NUM_COM;
                relPosition < LED_Driver_Gear_NUM_COM; 
                relPosition++)
            {
                if(relPosition < sizeLimit)
                {
                    LED_Driver_Gear_segMBuffer[relPosition] = LED_Driver_Gear_EncodeChar7Seg(character[i]);
                    i++;
                }
            }
        #endif /* (LED_Driver_Gear_NUM_SEG > 8u) */
    ...
    {
        #if (LED_Driver_Gear_NUM_SEG > 16u)
            sizeLimit = ((uint8)(strlen(character)) + position) - LED_Driver_Gear_TWO_NUM_COM;
            for(relPosition = position - LED_Driver_Gear_TWO_NUM_COM;
                relPosition < LED_Driver_Gear_NUM_COM; 
                relPosition++)
            {
                if(relPosition < sizeLimit)
                {
                    LED_Driver_Gear_segHBuffer[relPosition] = LED_Driver_Gear_EncodeChar7Seg(character[i]);
                    i++;
                }    
            }
        #endif /* (LED_Driver_Gear_NUM_SEG > 16u) */
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_PutChar7Seg
********************************************************************************
*
* Summary:
...

{
    ...
    {
        relPosition = position;
        LED_Driver_Gear_segLBuffer[relPosition] = LED_Driver_Gear_EncodeChar7Seg(character);
    }    
    else if (position < LED_Driver_Gear_TWO_NUM_COM)
    {
        #if (LED_Driver_Gear_NUM_SEG > 8u) 
            relPosition = position - LED_Driver_Gear_NUM_COM;
            LED_Driver_Gear_segMBuffer[relPosition] = LED_Driver_Gear_EncodeChar7Seg(character);
        #endif /* (LED_Driver_Gear_NUM_SEG > 8u) */
    }
    else
    {
            ...
            if(relPosition < LED_Driver_Gear_NUM_COM)
            {
                LED_Driver_Gear_segHBuffer[relPosition] = LED_Driver_Gear_EncodeChar7Seg(character);
            }
        #endif /* (LED_Driver_Gear_NUM_SEG > 16u) */
    }
}
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_Write7SegDigitDec
********************************************************************************
*
* Summary:
...

{
    ...
    if (digit < 10u)
    {
        ...
        {
            relPosition = position;
            LED_Driver_Gear_segLBuffer[relPosition] = LED_Driver_Gear_EncodeNumber7Seg(digit);
        }    
        else if (position < LED_Driver_Gear_TWO_NUM_COM)
        {
            #if (LED_Driver_Gear_NUM_SEG > 8u)
                relPosition = position - LED_Driver_Gear_NUM_COM;
                LED_Driver_Gear_segMBuffer[relPosition] = LED_Driver_Gear_EncodeNumber7Seg(digit);
            #endif /* (LED_Driver_Gear_NUM_SEG > 8u) */
        } 
        else
        {
            #if (LED_Driver_Gear_NUM_SEG > 16u)
                relPosition = position - LED_Driver_Gear_TWO_NUM_COM;
                if(relPosition < LED_Driver_Gear_TWO_NUM_COM)
                {
                    LED_Driver_Gear_segHBuffer[relPosition] = LED_Driver_Gear_EncodeNumber7Seg(digit);
                }
            #endif /* (LED_Driver_Gear_NUM_SEG > 16u) */
        }
    }
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_Write7SegDigitHex
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_Write14SegNumberDec
********************************************************************************
*
* Summary:
...

{
    ...
    {
        selectVal = (uint8)(numberVal % 10u);
        bufArray[i-1u] = LED_Driver_Gear_EncodeNumber14Seg(selectVal);
        numberVal = numberVal / 10u;
    }
    ...
    if(alignment == LED_Driver_Gear_ZERO_PAD)
    {
        ...
        {
            /* Place sign on the MSB */
            bufArray[0u] = LED_Driver_Gear_EncodeChar14Seg('-');
        }
    }
    else
    {
        for(i = 0u; i < digits; i++)
        {
            if (bufArray[i] != LED_Driver_Gear_ZERO_14CHAR)
            {
                if(numberSign == LED_Driver_Gear_SIGNED)
                {
                    if(i == 0u)
                    {
                        /* Place sign on the MSB */
                        bufArray[i] = LED_Driver_Gear_EncodeChar14Seg('-');
                    }
                    else
                    {
                        /* Place sign before MSB */
                        bufArray[i-1u] = LED_Driver_Gear_EncodeChar14Seg('-');
                    }
                }
                /* Exit when encountering non-zero number */
                break;
            ...
            else
            {
                ...
                {
                    /* Turn off the display */
                    bufArray[i] = LED_Driver_Gear_CLEAR_16;
                    zeroCount++;
                }
            }    
        }
    ...
    if(alignment == LED_Driver_Gear_LEFT_ALIGN)
    {
        for(i = 0u; i < digits; i++)
        {
            if(bufArray[i] != LED_Driver_Gear_CLEAR_16)
            {
                if(relPosition < LED_Driver_Gear_NUM_COM)
                {
                    /* Only show displays that are not empty */
                    LED_Driver_Gear_segLBuffer[relPosition] = LO8(bufArray[i]);
                #if (LED_Driver_Gear_NUM_SEG > 8u)
                    LED_Driver_Gear_segMBuffer[relPosition] = HI8(bufArray[i]);
                #endif /* (LED_Driver_Gear_NUM_SEG > 8u) */
                }
                relPosition++;
            }
    ...
    else
    {
        for(i = digits; i > 0u; i--)
        {
            if(((i + relPosition) - 1u) < LED_Driver_Gear_NUM_COM)
            {
                LED_Driver_Gear_segLBuffer[(i + relPosition) - 1u] = LO8(bufArray[i-1u]);
            #if (LED_Driver_Gear_NUM_SEG > 8u)
                LED_Driver_Gear_segMBuffer[(i + relPosition) - 1u] = HI8(bufArray[i-1u]);
            #endif /* (LED_Driver_Gear_NUM_SEG > 8u) */
            }
        }  
    }
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_Write14SegNumberHex
********************************************************************************
*
* Summary:
*  Displays a hexadecimal number up to 8 characters long, starting at 'position'
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_WriteString14Seg
********************************************************************************
*
* Summary:
...

{
    ...
    for(currPosition = position; currPosition < LED_Driver_Gear_NUM_COM; currPosition++)
    {
        if(currPosition < sizeLimit)
        {
            charDisplay = LED_Driver_Gear_EncodeChar14Seg(character[i]);
            LED_Driver_Gear_segLBuffer[currPosition] = LO8(charDisplay);
            #if (LED_Driver_Gear_NUM_SEG > 8u)
            LED_Driver_Gear_segMBuffer[currPosition] = HI8(charDisplay);
            #endif /* (LED_Driver_Gear_NUM_SEG > 8u) */
            i++;
        }
    }
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_PutChar14Seg
********************************************************************************
*
* Summary:
*  Displays an ascii encoded character at 'position'. This function can display
...

{
    ...
    if(position < LED_Driver_Gear_NUM_COM)
    {
        charDisplay = LED_Driver_Gear_EncodeChar14Seg(character);
        LED_Driver_Gear_segLBuffer[position] = LO8(charDisplay);
    #if (LED_Driver_Gear_NUM_SEG > 8u)
        LED_Driver_Gear_segMBuffer[position] = HI8(charDisplay);
    #endif /* (LED_Driver_Gear_NUM_SEG > 8u) */
    }
}
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_Write14SegDigitDec
********************************************************************************
*
* Summary:
*  Displays a single digit on the specified display. The number in 'digit' 
...

{
    ...
    if((digit < 10u) && (position < LED_Driver_Gear_NUM_COM))
    {
        charDisplay = LED_Driver_Gear_EncodeNumber14Seg(digit);
        LED_Driver_Gear_segLBuffer[position] = LO8(charDisplay);
    #if (LED_Driver_Gear_NUM_SEG > 8u)
        LED_Driver_Gear_segMBuffer[position] = HI8(charDisplay);
    #endif /* (LED_Driver_Gear_NUM_SEG > 8u) */
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_Write14SegDigitHex
********************************************************************************
*
* Summary:
*  Displays a single digit at the specified position. The number in 'digit' 
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_Write16SegNumberDec
********************************************************************************
*
* Summary:
*  Displays a signed integer up to 8 characters long, starting at 'position' 
...

{
    ...
    {
        selectVal = (uint8)(numberVal % 10u);
        bufArray[i-1u] = LED_Driver_Gear_EncodeNumber16Seg(selectVal);
        numberVal = numberVal / 10u;
    }
    ...
    if(alignment == LED_Driver_Gear_ZERO_PAD)
    {
        ...
        {
            /* Place sign on the MSB */
            bufArray[0u] = LED_Driver_Gear_EncodeChar16Seg('-');
        }
    }
    else
    {
        for(i = 0u; i < digits; i++)
        {
            if (bufArray[i] != LED_Driver_Gear_ZERO_16CHAR)
            {
                if(numberSign == LED_Driver_Gear_SIGNED)
                {
                    if(i == 0u)
                    {
                        /* Place sign on the MSB */
                        bufArray[i] = LED_Driver_Gear_EncodeChar16Seg('-');
                    }
                    else
                    {
                        /* Place sign before MSB */
                        bufArray[i-1u] = LED_Driver_Gear_EncodeChar16Seg('-');
                    }
                }
                /* Exit when encountering non-zero number */
                break;
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_Write16SegNumberHex
********************************************************************************
*
* Summary:
*  Displays a hexadecimal number up to 8 characters long, starting at 'position'
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_WriteString16Seg
********************************************************************************
*
* Summary:
*  Displays a null terminated string starting at 'position' and ending at either
...

{
    ...
    for(currPosition = position; currPosition < LED_Driver_Gear_NUM_COM; currPosition++)
    {
        if(currPosition < sizeLimit)
        {
            charDisplay = LED_Driver_Gear_EncodeChar16Seg(character[i]);
            LED_Driver_Gear_segLBuffer[currPosition] = LO8(charDisplay);
            #if (LED_Driver_Gear_NUM_SEG > 8u)
                LED_Driver_Gear_segMBuffer[currPosition] = HI8(charDisplay);
            #endif /* (LED_Driver_Gear_NUM_SEG > 8u) */
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_PutChar16Seg
********************************************************************************
*
* Summary:
*  Displays an ascii encoded character at 'position'. This function can display 
...

{
    ...
    if(position < LED_Driver_Gear_NUM_COM)
    {
        charDisplay = LED_Driver_Gear_EncodeChar16Seg(character);
        LED_Driver_Gear_segLBuffer[position] = LO8(charDisplay);
        #if (LED_Driver_Gear_NUM_SEG > 8u)
            LED_Driver_Gear_segMBuffer[position] = HI8(charDisplay);
        #endif /* (LED_Driver_Gear_NUM_SEG > 8u) */
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_Write16SegDigitDec
********************************************************************************
*
* Summary:
*  Displays a single digit on the specified display. The number in 'digit' 
...

{
    ...
    if((digit < 10u) && (position < LED_Driver_Gear_NUM_COM))
    {
        charDisplay = LED_Driver_Gear_EncodeNumber16Seg(digit);
        LED_Driver_Gear_segLBuffer[position] = LO8(charDisplay);
        #if (LED_Driver_Gear_NUM_SEG > 8u)
            LED_Driver_Gear_segMBuffer[position] = HI8(charDisplay);
        #endif /* (LED_Driver_Gear_NUM_SEG > 8u) */
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_Write16SegDigitHex
********************************************************************************
*
* Summary:
*  Displays a single digit at the specified position. The number in 'digit' 
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_PutDecimalPoint
********************************************************************************
*
* Summary:
...

{
    ...
    value = LED_Driver_Gear_DEC_POINT_VALUE;
    ...
    if(position < LED_Driver_Gear_NUM_COM)
    {
        ...
        {
            #if(LED_Driver_Gear_SEG_DRIVE == LED_Driver_Gear_HIGH)
                LED_Driver_Gear_segLBuffer[relPosition] &= ((uint8)(~(uint8)value));
            #else
                LED_Driver_Gear_segLBuffer[relPosition] |= ((uint8)(~(uint8)value));
            #endif /* (LED_Driver_Gear_SEG_DRIVE == LED_Driver_Gear_HIGH) */
        }
    ...
    else if (position < LED_Driver_Gear_TWO_NUM_COM)
    {
            ...
            {
                #if(LED_Driver_Gear_SEG_DRIVE == LED_Driver_Gear_HIGH)
                    LED_Driver_Gear_segMBuffer[relPosition] &= ((uint8)(~(uint8)value));
                #else
                    LED_Driver_Gear_segMBuffer[relPosition] |= ((uint8)(~(uint8)value));
                #endif /* (LED_Driver_Gear_SEG_DRIVE == LED_Driver_Gear_HIGH) */
            }
    ...
    else
    {
            ...
            if(relPosition < LED_Driver_Gear_NUM_COM)
            {
                ...
                {
                    #if(LED_Driver_Gear_SEG_DRIVE == LED_Driver_Gear_HIGH)
                        LED_Driver_Gear_segHBuffer[relPosition] &= ((uint8)(~(uint8)value));
                    #else
                        LED_Driver_Gear_segHBuffer[relPosition] |= ((uint8)(~(uint8)value));
                    #endif /* (LED_Driver_Gear_SEG_DRIVE == LED_Driver_Gear_HIGH) */
                }
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_GetDecimalPoint
********************************************************************************
*
* Summary:
...
uint8 LED_Driver_Gear_GetDecimalPoint(uint8 position) 
{
    ...
    {
        relPosition = position;
        decPointVal = (uint8)((uint8)(LED_Driver_Gear_segLBuffer[relPosition]) >> LED_Driver_Gear_DEC_POINT);
    } 
    else if (position < LED_Driver_Gear_TWO_NUM_COM)
    {
        #if (LED_Driver_Gear_NUM_SEG > 8u) 
            relPosition = position - LED_Driver_Gear_NUM_COM;
            decPointVal = (uint8)((uint8)(LED_Driver_Gear_segMBuffer[relPosition]) >> LED_Driver_Gear_DEC_POINT);
        #endif /* (LED_Driver_Gear_NUM_SEG > 8u) */
    }
    else
    {
            ...
            if(relPosition < LED_Driver_Gear_NUM_COM)
            {
                decPointVal = (uint8)((uint8)(LED_Driver_Gear_segHBuffer[relPosition]) >> LED_Driver_Gear_DEC_POINT);
            }
        #endif /* (LED_Driver_Gear_NUM_SEG > 16u) */
    }
    
    #if(LED_Driver_Gear_SEG_DRIVE == LED_Driver_Gear_HIGH)
        value = (uint8)(decPointVal & LED_Driver_Gear_DEC_POINT_MASK);
    #else
        value = (uint8)((uint8)((uint8)~decPointVal) & LED_Driver_Gear_DEC_POINT_MASK);
    #endif /* (LED_Driver_Gear_SEG_DRIVE == LED_Driver_Gear_HIGH) */
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_EncodeNumber7Seg
********************************************************************************
*
* Summary:
...
uint8 LED_Driver_Gear_EncodeNumber7Seg(uint8 number) 
{
    ...
    #if(LED_Driver_Gear_SEG_DRIVE == LED_Driver_Gear_HIGH)
        static const uint8 CYCODE LED_Driver_Gear_7SegDigits[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
    ...
    #endif /*  LED_Driver_Gear_SEG_DRIVE */

    uint8 valNum;
    uint8 valHex;
    
    valNum = (uint8)(number & LED_Driver_Gear_NIBBLE_MASK);
    
    valHex = LED_Driver_Gear_7SegDigits[valNum];
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_EncodeChar7Seg
********************************************************************************
*
* Summary:
...
uint8 LED_Driver_Gear_EncodeChar7Seg(char8 input)  
{
    /* 7-segment ASCII LUT */
    #if(LED_Driver_Gear_SEG_DRIVE == LED_Driver_Gear_HIGH)
        static const uint8 CYCODE LED_Driver_Gear_7SegASCII[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
    ...
    valLow = (uint8)((uint8)input & LED_Driver_Gear_NIBBLE_MASK);
    
    /* valHigh used to access the row of LUT */
    valHigh = (uint8)((uint8)input >> LED_Driver_Gear_SHIFT_NIBBLE);
    
    /* Check if the high nibble is within valid ASCII range */
    if ((valHigh > LED_Driver_Gear_SEG_ASCII_LIM_LO) && (valHigh < LED_Driver_Gear_SEG_ASCII_LIM_HI))
    {
        valHigh = valHigh - LED_Driver_Gear_SEG_ASCII_OFFSET;
        valHex = LED_Driver_Gear_7SegASCII[valHigh][valLow];
    }
    else
    {
        valHex = LED_Driver_Gear_CLEAR;
    }
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_EncodeNumber14Seg
********************************************************************************
*
* Summary:
...
uint16 LED_Driver_Gear_EncodeNumber14Seg(uint8 number) 
{
        ...
        static const uint8 CYCODE LED_Driver_Gear_14SegDigitsLow[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
        ...
        static const uint8 CYCODE LED_Driver_Gear_14SegDigitsHigh[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
    ...
    valHigh = (uint16)((uint16)LED_Driver_Gear_14SegDigitsHigh[valNum] << LED_Driver_Gear_SHIFT_BYTE);
    valLow = (uint16)LED_Driver_Gear_14SegDigitsLow[valNum];
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_EncodeChar14Seg
********************************************************************************
*
* Summary:
...
uint16 LED_Driver_Gear_EncodeChar14Seg(char8 input) 
{
        ...
        static const uint8 CYCODE LED_Driver_Gear_14SegASCIILow[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
        ...
        static const uint8 CYCODE LED_Driver_Gear_14SegASCIIHigh[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
    ...
    if ((valHigh > LED_Driver_Gear_SEG_ASCII_LIM_LO) && (valHigh < LED_Driver_Gear_SEG_ASCII_LIM_HI))
    {
        ...
        /* Get the 14-seg hex value from high and low byte LUTs */
        valHexHigh = (uint16)((uint16)LED_Driver_Gear_14SegASCIIHigh[valHigh][valLow] << LED_Driver_Gear_SHIFT_BYTE);
        valHexLow = (uint16)LED_Driver_Gear_14SegASCIILow[valHigh][valLow];
        
        valHex = valHexHigh | valHexLow;
    }
    else
    {
        valHex = LED_Driver_Gear_CLEAR_16;
    }
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_EncodeNumber16Seg
********************************************************************************
*
* Summary:
...
uint16 LED_Driver_Gear_EncodeNumber16Seg(uint8 number) 
{
        ...
        static const uint8 CYCODE LED_Driver_Gear_16SegDigitsLow[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
        ...
        static const uint8 CYCODE LED_Driver_Gear_16SegDigitsHigh[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
    ...
    valHigh = (uint16)((uint16)LED_Driver_Gear_16SegDigitsHigh[valNum] << LED_Driver_Gear_SHIFT_BYTE);
    valLow = (uint16)LED_Driver_Gear_16SegDigitsLow[valNum];
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_EncodeChar16Seg
********************************************************************************
*
* Summary:
...
uint16 LED_Driver_Gear_EncodeChar16Seg(char8 input) 
{
        ...
        static const uint8 CYCODE LED_Driver_Gear_16SegASCIILow[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
        ...
        static const uint8 CYCODE LED_Driver_Gear_16SegASCIIHigh[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
    ...
    if ((valHigh > LED_Driver_Gear_SEG_ASCII_LIM_LO) && (valHigh < LED_Driver_Gear_SEG_ASCII_LIM_HI))
    {
        ...
        /* Get the 16-seg hex value from high and low byte LUTs */
        valHexHigh = (uint16)((uint16)LED_Driver_Gear_16SegASCIIHigh[valHigh][valLow] << LED_Driver_Gear_SHIFT_BYTE);
        valHexLow = (uint16)LED_Driver_Gear_16SegASCIILow[valHigh][valLow];
...
/*******************************************************************************
* Function Name: LED_Driver_Gear_SetBrightness
********************************************************************************
*
* Summary:
...
{   
    #if(LED_Driver_Gear_COM_DRIVE == LED_Driver_Gear_HIGH)
        static const uint8 CYCODE LED_Driver_Gear_tempCom[16] = {0x00u, 0x01u, 0x00u, 0x02u, 0x00u, 0x04u,
                                                                  0x00u, 0x08u, 0x00u, 0x10u, 0x00u, 0x20u,
                                                                  0x00u, 0x40u, 0x00u, 0x80u};
    #else
        static const uint8 CYCODE LED_Driver_Gear_tempCom[16] = {0xFFu, 0xFEu, 0xFFu, 0xFDu, 0xFFu, 0xFBu,
    ...
    if(bright == 0u)
    {
        ...
        LED_Driver_Gear_comBuffer[(uint8)((uint8)position << 1u) + 1u] = LED_Driver_Gear_CLEAR;
    }
    else
    {
        /* Restore the commons */
        LED_Driver_Gear_comBuffer[(uint8)((uint8)position << 1u) + 1u]
            = LED_Driver_Gear_tempCom[(uint8)((uint8)position << 1u) + 1u];
    }
    
    /* Update the brightness register */
    LED_Driver_Gear_brightnessReg[position] = bright;
}


/*******************************************************************************
* Function Name: LED_Driver_Gear_GetBrightness
********************************************************************************
*
* Summary:
...
{
    return(LED_Driver_Gear_brightnessReg[position]);
}
