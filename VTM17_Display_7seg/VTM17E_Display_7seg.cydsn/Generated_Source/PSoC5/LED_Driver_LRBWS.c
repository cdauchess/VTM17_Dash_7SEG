...
#if (LED_Driver_LRBWS_BRIGHTNESS_EN == LED_Driver_LRBWS_ENABLED)
    #include <LED_Driver_LRBWS_DMA_BC_dma.h>
#endif /* (LED_Driver_LRBWS_BRIGHTNESS_EN == LED_Driver_LRBWS_ENABLED) */

/* Function prototypes */
static void LED_Driver_LRBWS_DmaInit(void) ;
static void LED_Driver_LRBWS_DmaEnable(void) ;
static void LED_Driver_LRBWS_DmaDisable(void) ;

/* Used to hold the common line output sequence */
#if(LED_Driver_LRBWS_COM_DRIVE == LED_Driver_LRBWS_HIGH)
    static uint8 LED_Driver_LRBWS_comBuffer[16] = {0x00u, 0x01u, 0x00u, 0x02u, 0x00u, 0x04u, 0x00u, 0x08u,
                                                    0x00u, 0x10u, 0x00u, 0x20u, 0x00u, 0x40u, 0x00u, 0x80u};
#else
    static uint8 LED_Driver_LRBWS_comBuffer[16] = {0xFFu, 0xFEu, 0xFFu, 0xFDu, 0xFFu, 0xFBu, 0xFFu, 0xF7u,
                                                    0xFFu, 0xEFu, 0xFFu, 0xDFu, 0xFFu, 0xBFu, 0xFFu, 0x7Fu};
#endif /* (LED_Driver_LRBWS_COM_DRIVE == LED_Driver_LRBWS_HIGH) */

/* Buffers used to hold the display(segment) values to be transferred to the control registers */
uint8 LED_Driver_LRBWS_segLBuffer[LED_Driver_LRBWS_NUM_COM];
#if (LED_Driver_LRBWS_NUM_SEG > 8u) 
    uint8 LED_Driver_LRBWS_segMBuffer[LED_Driver_LRBWS_NUM_COM];
#endif /* (LED_Driver_LRBWS_NUM_SEG > 8u) */
#if (LED_Driver_LRBWS_NUM_SEG > 16u) 
    uint8 LED_Driver_LRBWS_segHBuffer[LED_Driver_LRBWS_NUM_COM];
#endif /* (LED_Driver_LRBWS_NUM_SEG > 16u) */

#if (LED_Driver_LRBWS_BRIGHTNESS_EN == LED_Driver_LRBWS_ENABLED)
    /* The brightness register used to hold the brightness levels for each common */
    static uint8 LED_Driver_LRBWS_brightnessReg[LED_Driver_LRBWS_NUM_COM];
#endif /* (LED_Driver_LRBWS_BRIGHTNESS_EN == LED_Driver_LRBWS_ENABLED) */

/* DMA channel and TD declaractions */
static uint8 LED_Driver_LRBWS_dmaComChannel;
static uint8 LED_Driver_LRBWS_dmaComTd[LED_Driver_LRBWS_TWO_NUM_COM];

static uint8 LED_Driver_LRBWS_dmaSegChannel;
static uint8 LED_Driver_LRBWS_dmaSegLTd[LED_Driver_LRBWS_NUM_COM];

#if (LED_Driver_LRBWS_BRIGHTNESS_EN == LED_Driver_LRBWS_ENABLED)
    static uint8 LED_Driver_LRBWS_dmaBCChannel;
    static uint8 LED_Driver_LRBWS_dmaBCTd;
#endif /* (LED_Driver_LRBWS_BRIGHTNESS_EN == LED_Driver_LRBWS_ENABLED) */

uint8 LED_Driver_LRBWS_initVar = 0u;
uint8 LED_Driver_LRBWS_enableVar = 0u;


/*******************************************************************************
* Function Name: LED_Driver_LRBWS_DmaInit
********************************************************************************
*
* Summary: 
...
static void LED_Driver_LRBWS_DmaInit(void) 
{
    ...
    #if (LED_Driver_LRBWS_NUM_SEG > 8u)
        static uint8 LED_Driver_LRBWS_dmaSegMTd[LED_Driver_LRBWS_NUM_COM];
    #endif /* (LED_Driver_LRBWS_NUM_SEG > 8u) */
    #if (LED_Driver_LRBWS_NUM_SEG > 16u)
        static uint8 LED_Driver_LRBWS_dmaSegHTd[LED_Driver_LRBWS_NUM_COM];
    #endif /* (LED_Driver_LRBWS_NUM_SEG > 16u) */

    /************************* DMA configuration for the Commons *************************/
    /* Common DMA channel */
    LED_Driver_LRBWS_dmaComChannel = LED_Driver_LRBWS_DMA_Com_DmaInitialize(1u, 1u, 
                                        HI16(LED_Driver_LRBWS_SRAM_BASE), HI16(LED_Driver_LRBWS_CONTROL_REG_BASE));

    /* Check if the channel is valid */                                                                        
    if(LED_Driver_LRBWS_dmaComChannel != DMA_INVALID_CHANNEL)
    {
        /* Allocate the common TDs */
        for(i = 0u; i < LED_Driver_LRBWS_TWO_NUM_COM; i++)
        {
            LED_Driver_LRBWS_dmaComTd[i] = CyDmaTdAllocate();
        }
        
        /* Set configuration for the Common TDs */
        for(i = 0u;  i < (LED_Driver_LRBWS_TWO_NUM_COM - 1u); i++)
        {
            /* Separate configurations for odd and even TDs */
            if((uint8)(i & 1u) == 0u)
            {
                /* Set configuration for the Common TDs */
                (void) CyDmaTdSetConfiguration(LED_Driver_LRBWS_dmaComTd[i], 1u, 
                                                LED_Driver_LRBWS_dmaComTd[i+1u],
                                                TD_INC_SRC_ADR | LED_Driver_LRBWS_DMA_Com__TD_TERMOUT_EN);
            }
            else
            {
                /* Set configuration for the Common TDs */
                (void) CyDmaTdSetConfiguration(LED_Driver_LRBWS_dmaComTd[i], 1u,
                                                LED_Driver_LRBWS_dmaComTd[i+1u], TD_INC_SRC_ADR);
            }
        }
        /* Set the wraparound Common TD configuration */
        (void) CyDmaTdSetConfiguration(LED_Driver_LRBWS_dmaComTd[LED_Driver_LRBWS_TWO_NUM_COM - 1u], 
                                        1u, LED_Driver_LRBWS_dmaComTd[0u], TD_INC_SRC_ADR);
        
        /* Set the lower addresses of the common TDs */
        for(i = 0u; i < LED_Driver_LRBWS_TWO_NUM_COM; i++)
        {
            (void) CyDmaTdSetAddress(LED_Driver_LRBWS_dmaComTd[i],
                                        LO16((uint32) &LED_Driver_LRBWS_comBuffer[i]),
                                        LO16((uint32) LED_Driver_LRBWS_COM_CONTROL_PTR));
        }    
    }
    ...
    LED_Driver_LRBWS_dmaSegChannel = LED_Driver_LRBWS_DMA_Seg_DmaInitialize(1u, 1u, 
                                        HI16(LED_Driver_LRBWS_SRAM_BASE), HI16(LED_Driver_LRBWS_CONTROL_REG_BASE));
    
    /* Check if the segment channel is valid */
    if(LED_Driver_LRBWS_dmaSegChannel != DMA_INVALID_CHANNEL)
    {
        /* Allocate separate Tds for segments to implement the chain */
        for(i = 0u; i < LED_Driver_LRBWS_NUM_COM; i++)
        {
            LED_Driver_LRBWS_dmaSegLTd[i] = CyDmaTdAllocate();
            #if (LED_Driver_LRBWS_NUM_SEG > 8u)
                LED_Driver_LRBWS_dmaSegMTd[i] = CyDmaTdAllocate();
            #endif /* (LED_Driver_LRBWS_NUM_SEG > 8u) */
            #if (LED_Driver_LRBWS_NUM_SEG > 16u)
                LED_Driver_LRBWS_dmaSegHTd[i] = CyDmaTdAllocate();
            #endif /* (LED_Driver_LRBWS_NUM_SEG > 16u) */
        }
        
        /* Set the configuration for the Segment TDs */
        for(i = 0u; i < (LED_Driver_LRBWS_NUM_COM - 1u); i++)
        {
            #if (LED_Driver_LRBWS_NUM_SEG <= 8u)
                (void) CyDmaTdSetConfiguration(LED_Driver_LRBWS_dmaSegLTd[i], 1u, LED_Driver_LRBWS_dmaSegLTd[i+1u],
                                                TD_INC_SRC_ADR | LED_Driver_LRBWS_DMA_Seg__TD_TERMOUT_EN);
            #else    
                (void) CyDmaTdSetConfiguration(LED_Driver_LRBWS_dmaSegLTd[i], 1u, LED_Driver_LRBWS_dmaSegMTd[i],
                                                TD_INC_SRC_ADR | TD_AUTO_EXEC_NEXT);

                #if (LED_Driver_LRBWS_NUM_SEG <= 16u)
                    (void) CyDmaTdSetConfiguration(LED_Driver_LRBWS_dmaSegMTd[i], 1u, LED_Driver_LRBWS_dmaSegLTd[i+1u],
                                                    TD_INC_SRC_ADR | LED_Driver_LRBWS_DMA_Seg__TD_TERMOUT_EN);
                #else    
                    (void) CyDmaTdSetConfiguration(LED_Driver_LRBWS_dmaSegMTd[i], 1u, LED_Driver_LRBWS_dmaSegHTd[i],
                                                    TD_INC_SRC_ADR | TD_AUTO_EXEC_NEXT);
                    (void) CyDmaTdSetConfiguration(LED_Driver_LRBWS_dmaSegHTd[i], 1u, LED_Driver_LRBWS_dmaSegLTd[i+1u],
                                                    TD_INC_SRC_ADR | LED_Driver_LRBWS_DMA_Seg__TD_TERMOUT_EN);
                #endif /* LED_Driver_LRBWS_NUM_SEG <= 16u */
            #endif /* LED_Driver_LRBWS_NUM_SEG <= 8u */
        }
        ...
        #if (LED_Driver_LRBWS_NUM_SEG <= 8u)
            (void) CyDmaTdSetConfiguration(LED_Driver_LRBWS_dmaSegLTd[LED_Driver_LRBWS_NUM_COM - 1u],
                                            1u, LED_Driver_LRBWS_dmaSegLTd[0u], 
                                            TD_INC_SRC_ADR | LED_Driver_LRBWS_DMA_Seg__TD_TERMOUT_EN);
        #else    
            (void) CyDmaTdSetConfiguration(LED_Driver_LRBWS_dmaSegLTd[LED_Driver_LRBWS_NUM_COM - 1u],
                                            1u, LED_Driver_LRBWS_dmaSegMTd[LED_Driver_LRBWS_NUM_COM - 1u],
                                            TD_INC_SRC_ADR | TD_AUTO_EXEC_NEXT);
            #if (LED_Driver_LRBWS_NUM_SEG <= 16u)
                (void) CyDmaTdSetConfiguration(LED_Driver_LRBWS_dmaSegMTd[LED_Driver_LRBWS_NUM_COM - 1u],
                                                1u, LED_Driver_LRBWS_dmaSegLTd[0u], 
                                                TD_INC_SRC_ADR | LED_Driver_LRBWS_DMA_Seg__TD_TERMOUT_EN);
            #else    
                (void) CyDmaTdSetConfiguration(LED_Driver_LRBWS_dmaSegMTd[LED_Driver_LRBWS_NUM_COM - 1u],
                                                1u, LED_Driver_LRBWS_dmaSegHTd[LED_Driver_LRBWS_NUM_COM - 1u], 
                                                TD_INC_SRC_ADR | TD_AUTO_EXEC_NEXT);
                (void) CyDmaTdSetConfiguration(LED_Driver_LRBWS_dmaSegHTd[LED_Driver_LRBWS_NUM_COM - 1u],
                                                1u, LED_Driver_LRBWS_dmaSegLTd[0u], 
                                                TD_INC_SRC_ADR | LED_Driver_LRBWS_DMA_Seg__TD_TERMOUT_EN);
            #endif /* (LED_Driver_LRBWS_NUM_SEG <= 16u) */
        #endif /* (LED_Driver_LRBWS_NUM_SEG <= 8u) */
        ...
        for(i = 0u; i < LED_Driver_LRBWS_NUM_COM; i++)
        {
            (void) CyDmaTdSetAddress(LED_Driver_LRBWS_dmaSegLTd[i],
                                            LO16((uint32) &LED_Driver_LRBWS_segLBuffer[i]),
                                            LO16((uint32) LED_Driver_LRBWS_SEG_L_CONTROL_PTR));
            #if (LED_Driver_LRBWS_NUM_SEG > 8u)
                (void) CyDmaTdSetAddress(LED_Driver_LRBWS_dmaSegMTd[i],
                                            LO16((uint32) &LED_Driver_LRBWS_segMBuffer[i]),
                                            LO16((uint32) LED_Driver_LRBWS_SEG_M_CONTROL_PTR));
            #endif /* (LED_Driver_LRBWS_NUM_SEG > 8u) */
            #if (LED_Driver_LRBWS_NUM_SEG > 16u)
                (void) CyDmaTdSetAddress(LED_Driver_LRBWS_dmaSegHTd[i], 
                                            LO16((uint32) &LED_Driver_LRBWS_segHBuffer[i]),
                                            LO16((uint32) LED_Driver_LRBWS_SEG_H_CONTROL_PTR));
            #endif /* (LED_Driver_LRBWS_NUM_SEG > 16u) */
        }
    }
    ...
    /******************** DMA configuration for Brightness control *********************/
        ...
        LED_Driver_LRBWS_dmaBCChannel = LED_Driver_LRBWS_DMA_BC_DmaInitialize(1u, 1u, 
                                        HI16(LED_Driver_LRBWS_SRAM_BASE), HI16(LED_Driver_LRBWS_CONTROL_REG_BASE));
            
        /* Check if the channels are valid */
        if(LED_Driver_LRBWS_dmaBCChannel != DMA_INVALID_CHANNEL)
        {
            LED_Driver_LRBWS_dmaBCTd = CyDmaTdAllocate();
            
            /* Set the TD configuration for the brightness DMA */
            (void) CyDmaTdSetConfiguration(LED_Driver_LRBWS_dmaBCTd, LED_Driver_LRBWS_NUM_COM,
                                        LED_Driver_LRBWS_dmaBCTd, TD_INC_SRC_ADR);
            
            /* Lower address for the brightness Td */
            (void) CyDmaTdSetAddress(LED_Driver_LRBWS_dmaBCTd, LO16((uint32) &LED_Driver_LRBWS_brightnessReg[0u]),
                                                               LO16((uint32)LED_Driver_LRBWS_COMPARE_PTR));
        }
    #endif /* (LED_Driver_LRBWS_BRIGHTNESS_EN == LED_Driver_LRBWS_ENABLED) */
}
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_DmaEnable
********************************************************************************
*
* Summary: 
...
static void LED_Driver_LRBWS_DmaEnable(void) 
{
    ...
    (void) CyDmaChSetInitialTd(LED_Driver_LRBWS_dmaComChannel, LED_Driver_LRBWS_dmaComTd[0u]);
    (void) CyDmaChSetInitialTd(LED_Driver_LRBWS_dmaSegChannel, LED_Driver_LRBWS_dmaSegLTd[0u]);
    #if (LED_Driver_LRBWS_BRIGHTNESS_EN == LED_Driver_LRBWS_ENABLED)
        (void) CyDmaChSetInitialTd(LED_Driver_LRBWS_dmaBCChannel, LED_Driver_LRBWS_dmaBCTd);
    #endif /* (LED_Driver_LRBWS_BRIGHTNESS_EN == LED_Driver_LRBWS_ENABLED) */
    
    /* Enable all channels */
    (void) CyDmaChEnable(LED_Driver_LRBWS_dmaComChannel, 1u);
    (void) CyDmaChEnable(LED_Driver_LRBWS_dmaSegChannel, 1u);
    #if (LED_Driver_LRBWS_BRIGHTNESS_EN == LED_Driver_LRBWS_ENABLED)
        (void) CyDmaChEnable(LED_Driver_LRBWS_dmaBCChannel, 1u);
    #endif /* (LED_Driver_LRBWS_BRIGHTNESS_EN == LED_Driver_LRBWS_ENABLED) */
}


/*******************************************************************************
* Function Name: LED_Driver_LRBWS_DmaDisable
********************************************************************************
*
* Summary: 
...
static void LED_Driver_LRBWS_DmaDisable(void) 
{
    uint8 i;
    uint8 channel;
    uint8 dmaChannelList[LED_Driver_LRBWS_CHANNEL_LIMIT];
    
    dmaChannelList[0u] = LED_Driver_LRBWS_dmaComChannel;
    dmaChannelList[1u] = LED_Driver_LRBWS_dmaSegChannel;
    #if (LED_Driver_LRBWS_BRIGHTNESS_EN == LED_Driver_LRBWS_ENABLED)
        dmaChannelList[2u] = LED_Driver_LRBWS_dmaBCChannel;
    #endif /* (LED_Driver_LRBWS_BRIGHTNESS_EN == LED_Driver_LRBWS_ENABLED) */

    for(i = 0u; i < LED_Driver_LRBWS_CHANNEL_LIMIT; i++)
    {
        channel = dmaChannelList[i];
        (void) CyDmaChDisable(channel);
        
        LED_Driver_LRBWS_Com_Driver_Write(LED_Driver_LRBWS_CLEAR);
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_Init
********************************************************************************
*
* Summary: 
...
*
*******************************************************************************/
void LED_Driver_LRBWS_Init(void) 
{
    static uint8 LED_Driver_LRBWS_dmaInitDone = 0u;
    #if (LED_Driver_LRBWS_BRIGHTNESS_EN == LED_Driver_LRBWS_ENABLED)
        uint8 i;
    #endif /* (LED_Driver_LRBWS_BRIGHTNESS_EN == LED_Driver_LRBWS_ENABLED) */
    
    /* Start fresh by clearing the display */
    LED_Driver_LRBWS_ClearDisplayAll();
    
    /* Initialize the DMAs */
    if(0u == LED_Driver_LRBWS_dmaInitDone)
    {
        LED_Driver_LRBWS_DmaInit();
        LED_Driver_LRBWS_dmaInitDone = 1u;
    }
    ...
    /* Set the brightness to full */
        ...
        for(i = 0u; i<(LED_Driver_LRBWS_NUM_COM); i++)
        {
            LED_Driver_LRBWS_brightnessReg[i] = LED_Driver_LRBWS_INIT_BRIGHT;
        }
    #endif /* (LED_Driver_LRBWS_BRIGHTNESS_EN == LED_Driver_LRBWS_ENABLED) */
}
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_Enable
********************************************************************************
*
* Summary: 
...
*
*******************************************************************************/
void LED_Driver_LRBWS_Enable(void) 
{
    /* Enable the DMAs */
    LED_Driver_LRBWS_DmaEnable();
 
    #if (LED_Driver_LRBWS_BRIGHTNESS_EN == LED_Driver_LRBWS_ENABLED)
       /* Initialize to zero */
       LED_Driver_LRBWS_COUNTER_REG = 0x00u;
       LED_Driver_LRBWS_COMPARE_REG = 0x00u;
       
        /* Enable the PWM for brightness control */
       LED_Driver_LRBWS_CONTROL_REG = LED_Driver_LRBWS_CTRL_ENABLE;
    #endif /* (LED_Driver_LRBWS_BRIGHTNESS_EN == LED_Driver_LRBWS_ENABLED) */
    
    LED_Driver_LRBWS_enableVar = LED_Driver_LRBWS_CTRL_ENABLE;
}


/*******************************************************************************
* Function Name: LED_Driver_LRBWS_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LED_Driver_LRBWS_Start(void) 
{
    if(0u == LED_Driver_LRBWS_initVar)
    {
        LED_Driver_LRBWS_Init();
        LED_Driver_LRBWS_initVar = 1u;
    }
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LED_Driver_LRBWS_Stop(void) 
{
    #if (LED_Driver_LRBWS_BRIGHTNESS_EN == LED_Driver_LRBWS_ENABLED)
        /* Disable the PWM */
        LED_Driver_LRBWS_CONTROL_REG = LED_Driver_LRBWS_CTRL_DISABLE;
    #endif /* (LED_Driver_LRBWS_BRIGHTNESS_EN == LED_Driver_LRBWS_ENABLED) */
    
    LED_Driver_LRBWS_DmaDisable();
    LED_Driver_LRBWS_ClearDisplayAll();
    
    LED_Driver_LRBWS_enableVar  = LED_Driver_LRBWS_CTRL_DISABLE;
}


/*******************************************************************************
* Function Name: LED_Driver_LRBWS_SetDisplayRAM
********************************************************************************
*
* Summary:
...
*  RAM directly by writing to LED_Driver_LRBWS_segLBuffer,
*  LED_Driver_LRBWS_segMBuffer, LED_Driver_LRBWS_segHBuffer.
*
* Parameters:
...
*
*******************************************************************************/
void LED_Driver_LRBWS_SetDisplayRAM(uint8 value, uint8 position) 
{
    uint8 relPosition;
    
    if(position < LED_Driver_LRBWS_NUM_COM)
    {
        relPosition = position;
        #if(LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH)
            LED_Driver_LRBWS_segLBuffer[relPosition] = value;
        #else
            LED_Driver_LRBWS_segLBuffer[relPosition] = ((uint8) ~value);
        #endif /* LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH */
    }    
    else if (position < LED_Driver_LRBWS_TWO_NUM_COM)
    {
        #if (LED_Driver_LRBWS_NUM_SEG > 8u)
            relPosition = position - LED_Driver_LRBWS_NUM_COM;
            #if(LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH)
                LED_Driver_LRBWS_segMBuffer[relPosition] = value;
            #else
                LED_Driver_LRBWS_segMBuffer[relPosition] = ((uint8) ~value);
            #endif /* LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH */
        #endif /* (LED_Driver_LRBWS_NUM_SEG > 8u) */
    }
    else
    {
        #if (LED_Driver_LRBWS_NUM_SEG > 16u)
            relPosition = position - LED_Driver_LRBWS_TWO_NUM_COM;
            if(relPosition < LED_Driver_LRBWS_NUM_COM)
            {
                #if(LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH)
                    LED_Driver_LRBWS_segHBuffer[relPosition] = value;
                #else
                    LED_Driver_LRBWS_segMBuffer[relPosition] = ((uint8) ~value);
                #endif /* LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH */
            }
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_SetRC
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LED_Driver_LRBWS_SetRC(uint8 row, uint8 column) 
{
    uint8 rowSeg;
    
    if(column < LED_Driver_LRBWS_NUM_COM)
    {
        if(row < 8u)
        {
            rowSeg = row;
            #if(LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH)
                LED_Driver_LRBWS_segLBuffer[column] |= ((uint8)((uint8)0x01u << rowSeg));
            #else
                LED_Driver_LRBWS_segLBuffer[column] &= ((uint8)~((uint8)((uint8)0x01u << rowSeg)));
            #endif /* LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH */
        }
        else if (row < 16u)
        {
                ...
                rowSeg = row - 8u;
                #if(LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH)
                    LED_Driver_LRBWS_segMBuffer[column] |= ((uint8)((uint8)0x01u << rowSeg));
                #else
                    LED_Driver_LRBWS_segMBuffer[column] &= ((uint8)~((uint8)((uint8)0x01u << rowSeg)));
                #endif /* LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH */
            #endif /* (LED_Driver_LRBWS_NUM_SEG > 8u) */
        }
        else
        {
                ...
                rowSeg = row - 16u;
                #if(LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH)
                    LED_Driver_LRBWS_segHBuffer[column] |= ((uint8)((uint8)0x01u << rowSeg));
                #else
                    LED_Driver_LRBWS_segHBuffer[column] &= ((uint8)~((uint8)((uint8)0x01u << rowSeg)));
                #endif /* LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH */
            #endif /* (LED_Driver_LRBWS_NUM_SEG > 16u) */
        }
    }
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_ClearRC
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LED_Driver_LRBWS_ClearRC(uint8 row, uint8 column) 
{
    uint8 rowSeg;
    uint8 rowSelect;
    ...
    if(column < LED_Driver_LRBWS_NUM_COM)
    {
        if(row < 8u)
        {
            ...
            #if(LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH)
                LED_Driver_LRBWS_segLBuffer[column] &= ((uint8)(~(uint8)rowSelect));
            #else
                LED_Driver_LRBWS_segLBuffer[column] |= rowSelect;
            #endif /* LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH */
        }    
        else if (row < 16u)
        {
                ...
                rowSelect = ((uint8)((uint8)0x01u << rowSeg));
                #if(LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH)
                    LED_Driver_LRBWS_segMBuffer[column] &= ((uint8)(~(uint8)rowSelect));
                #else
                    LED_Driver_LRBWS_segMBuffer[column] |= rowSelect;
                #endif /* LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH */
            #endif /* (LED_Driver_LRBWS_NUM_SEG > 8u) */
        }
        else
        {
                ...
                rowSelect = ((uint8)((uint8)0x01u << rowSeg));
                #if(LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH)
                    LED_Driver_LRBWS_segHBuffer[column] &= ((uint8)(~(uint8)rowSelect));
                #else
                    LED_Driver_LRBWS_segHBuffer[column] |= rowSelect;
                #endif /* LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH */
            #endif /* (LED_Driver_LRBWS_NUM_SEG > 16u) */
        }
    }
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_ToggleRC
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LED_Driver_LRBWS_ToggleRC(uint8 row, uint8 column) 
{
    uint8 rowSeg;
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_GetRC
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 LED_Driver_LRBWS_GetRC(uint8 row, uint8 column) 
{
    uint8 valueRC = 0u;
    uint8 rowSeg;
    ...
    if(column < LED_Driver_LRBWS_NUM_COM)
    {
        ...
        {
            rowSeg = row;
            rowSegSelect = (uint8)((uint8)LED_Driver_LRBWS_segLBuffer[column] >> rowSeg);
        }    
        else if (row < 16u)
        {
            #if (LED_Driver_LRBWS_NUM_SEG > 8u) 
                rowSeg = row - 8u;
                rowSegSelect = (uint8)((uint8)LED_Driver_LRBWS_segMBuffer[column] >> rowSeg);
            #endif /* (LED_Driver_LRBWS_NUM_SEG > 8u) */
        }
        else
        {
            #if (LED_Driver_LRBWS_NUM_SEG > 16u)
                rowSeg = row - 16u;
                rowSegSelect = (uint8)((uint8)LED_Driver_LRBWS_segHBuffer[column] >> rowSeg);
            #endif /* (LED_Driver_LRBWS_NUM_SEG > 16u) */
        }
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_ClearDisplay
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LED_Driver_LRBWS_ClearDisplay(uint8 position) 
{  
    uint8 relPosition;
    ...
    {
        relPosition = position;
        LED_Driver_LRBWS_segLBuffer[relPosition] = LED_Driver_LRBWS_CLEAR;
    }    
    else if (position < LED_Driver_LRBWS_TWO_NUM_COM)
    {
        #if (LED_Driver_LRBWS_NUM_SEG > 8u)
            relPosition = position - LED_Driver_LRBWS_NUM_COM;
            LED_Driver_LRBWS_segMBuffer[relPosition] = LED_Driver_LRBWS_CLEAR;
        #endif /* (LED_Driver_LRBWS_NUM_SEG > 8u) */
    }
    else
    {
            ...
            if(relPosition < LED_Driver_LRBWS_NUM_COM)
            {
                LED_Driver_LRBWS_segHBuffer[relPosition] = LED_Driver_LRBWS_CLEAR;
            }
        #endif /* (LED_Driver_LRBWS_NUM_SEG > 16u) */
    }
}
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_ClearDisplayAll
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LED_Driver_LRBWS_ClearDisplayAll(void) 
{
    uint8 position;
    
    for(position = 0u; position < LED_Driver_LRBWS_NUM_COM; position++)
    {
        LED_Driver_LRBWS_segLBuffer[position] = LED_Driver_LRBWS_CLEAR; 
        
        #if (LED_Driver_LRBWS_NUM_SEG > 8u) 
            LED_Driver_LRBWS_segMBuffer[position] = LED_Driver_LRBWS_CLEAR;
        #endif /* (LED_Driver_LRBWS_NUM_SEG > 8u) */
        
        #if (LED_Driver_LRBWS_NUM_SEG > 16u)
            LED_Driver_LRBWS_segHBuffer[position] = LED_Driver_LRBWS_CLEAR;
        #endif /* (LED_Driver_LRBWS_NUM_SEG > 16u) */
    }
}
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_Write7SegNumberDec
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LED_Driver_LRBWS_Write7SegNumberDec(int32 number, uint8 position, uint8 digits, uint8 alignment)

{
    ...
    if(position < LED_Driver_LRBWS_NUM_COM)
    {
        choiceReg = LED_Driver_LRBWS_SEG_REG_LOW;
        relPosition = position;
    }
    else if (position < LED_Driver_LRBWS_TWO_NUM_COM)
    {
        choiceReg = LED_Driver_LRBWS_SEG_REG_MID;
        relPosition = position - LED_Driver_LRBWS_NUM_COM;
    }
    else
    {
        choiceReg = LED_Driver_LRBWS_SEG_REG_HIGH;
        relPosition = position - LED_Driver_LRBWS_TWO_NUM_COM;
    }
    
    /* Routine to extract the sign and to convert to unsigned */
    if(number >= LED_Driver_LRBWS_ZERO_NEG)
    {
        numberVal = (uint32)number;
        numberSign = LED_Driver_LRBWS_UNSIGNED;
    }
    else
    {
        numberVal = (uint32)(-number);
        numberSign = LED_Driver_LRBWS_SIGNED;
    }
    ...
    for(i = digits; i > 0u; i--)
    {
        ...
        bufArray[i-1u] = LED_Driver_LRBWS_EncodeNumber7Seg(selectVal);
        numberVal = numberVal / 10u;
    }
    
    /* Clean up the array and place sign if needed */
    if(alignment == LED_Driver_LRBWS_ZERO_PAD)
    {
        if(numberSign == LED_Driver_LRBWS_SIGNED)
        {
            /* Place sign on the MSB */
            bufArray[0u] = LED_Driver_LRBWS_EncodeChar7Seg('-');
        }
    }
    else
    {
        for(i = 0u; i < digits; i++)
        {
            if (bufArray[i] != LED_Driver_LRBWS_ZERO_7CHAR)
            {
                if(numberSign == LED_Driver_LRBWS_SIGNED)
                {
                    if(i == 0u)
                    {
                        /* Place sign on the MSB */
                        bufArray[i] = LED_Driver_LRBWS_EncodeChar7Seg('-');
                    }
                    else
                    {
                        /* Place sign before MSB */
                        bufArray[i-1u] = LED_Driver_LRBWS_EncodeChar7Seg('-');
                    }
                }
            ...
            else
            {
                if(zeroCount != (digits - 1u))
                {
                    ...
                    bufArray[i] = LED_Driver_LRBWS_CLEAR;
                    zeroCount++;
                }
            }    
        }
    ...
    if(alignment == LED_Driver_LRBWS_LEFT_ALIGN)
    {
        for(i = 0u; i < digits; i++)
        {
            if(bufArray[i] != LED_Driver_LRBWS_CLEAR)
            {
                if(relPosition < LED_Driver_LRBWS_NUM_COM)
                {
                    ...
                    switch(choiceReg)
                    {
                        case LED_Driver_LRBWS_SEG_REG_LOW :
                            LED_Driver_LRBWS_segLBuffer[relPosition] = bufArray[i];
                            break;
                        case LED_Driver_LRBWS_SEG_REG_MID :
                            #if (LED_Driver_LRBWS_NUM_SEG > 8u)
                                LED_Driver_LRBWS_segMBuffer[relPosition] = bufArray[i];
                            #endif /* (LED_Driver_LRBWS_NUM_SEG > 8u) */
                            break;
                        case LED_Driver_LRBWS_SEG_REG_HIGH :
                            #if (LED_Driver_LRBWS_NUM_SEG > 16u)
                                LED_Driver_LRBWS_segHBuffer[relPosition] = bufArray[i];
                            #endif /* (LED_Driver_LRBWS_NUM_SEG > 16u) */
                            break;
                        default :
                            LED_Driver_LRBWS_segLBuffer[relPosition] = bufArray[i];
    ...
    else
    {
        ...
        for(i = digits; i > 0u; i--)
        {
            if(((i + relPosition) - 1u) < LED_Driver_LRBWS_NUM_COM)
            {
                /* Decide which row to use */
                switch(choiceReg)
                {
                    case LED_Driver_LRBWS_SEG_REG_LOW :
                        LED_Driver_LRBWS_segLBuffer[(i + relPosition) - 1u] = bufArray[i-1u];
                        break;
                    case LED_Driver_LRBWS_SEG_REG_MID :
                        #if (LED_Driver_LRBWS_NUM_SEG > 8u)
                            LED_Driver_LRBWS_segMBuffer[(i + relPosition) - 1u] = bufArray[i-1u];
                        #endif /* (LED_Driver_LRBWS_NUM_SEG > 8u) */
                        break;
                    case LED_Driver_LRBWS_SEG_REG_HIGH :
                        #if (LED_Driver_LRBWS_NUM_SEG > 16u)
                            LED_Driver_LRBWS_segHBuffer[(i + relPosition) - 1u] = bufArray[i-1u];
                        #endif /* (LED_Driver_LRBWS_NUM_SEG > 16u) */
                        break;
                    default :
                        LED_Driver_LRBWS_segLBuffer[(i + relPosition) - 1u] = bufArray[i-1u];
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_Write7SegNumberHex
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LED_Driver_LRBWS_Write7SegNumberHex(uint32 number, uint8 position, uint8 digits, uint8 alignment)

{
    ...
    for(i = digits; i > 0u; i--)
    {
        selectVal = (uint8)((uint8)numberVal & LED_Driver_LRBWS_NIBBLE_MASK);
        bufArray[i-1u] = LED_Driver_LRBWS_EncodeNumber7Seg(selectVal);
        numberVal = (uint32)((uint32)numberVal >> LED_Driver_LRBWS_SHIFT_NIBBLE);
    }
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_WriteString7Seg
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LED_Driver_LRBWS_WriteString7Seg(char8 const character[], uint8 position)

{
    ...
    if(position < LED_Driver_LRBWS_NUM_COM)
    {
        ...
        for(relPosition = position; relPosition < LED_Driver_LRBWS_NUM_COM; relPosition++)
        {
            if(relPosition < sizeLimit)
            {
                LED_Driver_LRBWS_segLBuffer[relPosition] = LED_Driver_LRBWS_EncodeChar7Seg(character[i]);
                i++;
            }
        }
    }    
    ...
    {
        #if (LED_Driver_LRBWS_NUM_SEG > 8u) 
            sizeLimit = ((uint8)(strlen(character)) + position) - LED_Driver_LRBWS_NUM_COM;
            for(relPosition = position - LED_Driver_LRBWS_NUM_COM;
                relPosition < LED_Driver_LRBWS_NUM_COM; 
                relPosition++)
            {
                if(relPosition < sizeLimit)
                {
                    LED_Driver_LRBWS_segMBuffer[relPosition] = LED_Driver_LRBWS_EncodeChar7Seg(character[i]);
                    i++;
                }
            }
        #endif /* (LED_Driver_LRBWS_NUM_SEG > 8u) */
    ...
    {
        #if (LED_Driver_LRBWS_NUM_SEG > 16u)
            sizeLimit = ((uint8)(strlen(character)) + position) - LED_Driver_LRBWS_TWO_NUM_COM;
            for(relPosition = position - LED_Driver_LRBWS_TWO_NUM_COM;
                relPosition < LED_Driver_LRBWS_NUM_COM; 
                relPosition++)
            {
                if(relPosition < sizeLimit)
                {
                    LED_Driver_LRBWS_segHBuffer[relPosition] = LED_Driver_LRBWS_EncodeChar7Seg(character[i]);
                    i++;
                }    
            }
        #endif /* (LED_Driver_LRBWS_NUM_SEG > 16u) */
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_PutChar7Seg
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LED_Driver_LRBWS_PutChar7Seg(char8 character , uint8 position)

{
    ...
    {
        relPosition = position;
        LED_Driver_LRBWS_segLBuffer[relPosition] = LED_Driver_LRBWS_EncodeChar7Seg(character);
    }    
    else if (position < LED_Driver_LRBWS_TWO_NUM_COM)
    {
        #if (LED_Driver_LRBWS_NUM_SEG > 8u) 
            relPosition = position - LED_Driver_LRBWS_NUM_COM;
            LED_Driver_LRBWS_segMBuffer[relPosition] = LED_Driver_LRBWS_EncodeChar7Seg(character);
        #endif /* (LED_Driver_LRBWS_NUM_SEG > 8u) */
    }
    else
    {
            ...
            if(relPosition < LED_Driver_LRBWS_NUM_COM)
            {
                LED_Driver_LRBWS_segHBuffer[relPosition] = LED_Driver_LRBWS_EncodeChar7Seg(character);
            }
        #endif /* (LED_Driver_LRBWS_NUM_SEG > 16u) */
    }
}
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_Write7SegDigitDec
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LED_Driver_LRBWS_Write7SegDigitDec(uint8 digit, uint8 position)

{
    ...
    if (digit < 10u)
    {
        ...
        {
            relPosition = position;
            LED_Driver_LRBWS_segLBuffer[relPosition] = LED_Driver_LRBWS_EncodeNumber7Seg(digit);
        }    
        else if (position < LED_Driver_LRBWS_TWO_NUM_COM)
        {
            #if (LED_Driver_LRBWS_NUM_SEG > 8u)
                relPosition = position - LED_Driver_LRBWS_NUM_COM;
                LED_Driver_LRBWS_segMBuffer[relPosition] = LED_Driver_LRBWS_EncodeNumber7Seg(digit);
            #endif /* (LED_Driver_LRBWS_NUM_SEG > 8u) */
        } 
        else
        {
            #if (LED_Driver_LRBWS_NUM_SEG > 16u)
                relPosition = position - LED_Driver_LRBWS_TWO_NUM_COM;
                if(relPosition < LED_Driver_LRBWS_TWO_NUM_COM)
                {
                    LED_Driver_LRBWS_segHBuffer[relPosition] = LED_Driver_LRBWS_EncodeNumber7Seg(digit);
                }
            #endif /* (LED_Driver_LRBWS_NUM_SEG > 16u) */
        }
    }
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_Write7SegDigitHex
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LED_Driver_LRBWS_Write7SegDigitHex(uint8 digit, uint8 position)
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_Write14SegNumberDec
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LED_Driver_LRBWS_Write14SegNumberDec(int32 number, uint8 position, uint8 digits, uint8 alignment)

{
    ...
    {
        selectVal = (uint8)(numberVal % 10u);
        bufArray[i-1u] = LED_Driver_LRBWS_EncodeNumber14Seg(selectVal);
        numberVal = numberVal / 10u;
    }
    ...
    if(alignment == LED_Driver_LRBWS_ZERO_PAD)
    {
        ...
        {
            /* Place sign on the MSB */
            bufArray[0u] = LED_Driver_LRBWS_EncodeChar14Seg('-');
        }
    }
    else
    {
        for(i = 0u; i < digits; i++)
        {
            if (bufArray[i] != LED_Driver_LRBWS_ZERO_14CHAR)
            {
                if(numberSign == LED_Driver_LRBWS_SIGNED)
                {
                    if(i == 0u)
                    {
                        /* Place sign on the MSB */
                        bufArray[i] = LED_Driver_LRBWS_EncodeChar14Seg('-');
                    }
                    else
                    {
                        /* Place sign before MSB */
                        bufArray[i-1u] = LED_Driver_LRBWS_EncodeChar14Seg('-');
                    }
                }
                /* Exit when encountering non-zero number */
                break;
            ...
            else
            {
                ...
                {
                    /* Turn off the display */
                    bufArray[i] = LED_Driver_LRBWS_CLEAR_16;
                    zeroCount++;
                }
            }    
        }
    ...
    if(alignment == LED_Driver_LRBWS_LEFT_ALIGN)
    {
        for(i = 0u; i < digits; i++)
        {
            if(bufArray[i] != LED_Driver_LRBWS_CLEAR_16)
            {
                if(relPosition < LED_Driver_LRBWS_NUM_COM)
                {
                    /* Only show displays that are not empty */
                    LED_Driver_LRBWS_segLBuffer[relPosition] = LO8(bufArray[i]);
                #if (LED_Driver_LRBWS_NUM_SEG > 8u)
                    LED_Driver_LRBWS_segMBuffer[relPosition] = HI8(bufArray[i]);
                #endif /* (LED_Driver_LRBWS_NUM_SEG > 8u) */
                }
                relPosition++;
            }
    ...
    else
    {
        for(i = digits; i > 0u; i--)
        {
            if(((i + relPosition) - 1u) < LED_Driver_LRBWS_NUM_COM)
            {
                LED_Driver_LRBWS_segLBuffer[(i + relPosition) - 1u] = LO8(bufArray[i-1u]);
            #if (LED_Driver_LRBWS_NUM_SEG > 8u)
                LED_Driver_LRBWS_segMBuffer[(i + relPosition) - 1u] = HI8(bufArray[i-1u]);
            #endif /* (LED_Driver_LRBWS_NUM_SEG > 8u) */
            }
        }  
    }
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_Write14SegNumberHex
********************************************************************************
*
* Summary:
*  Displays a hexadecimal number up to 8 characters long, starting at 'position'
...
*
*******************************************************************************/
void LED_Driver_LRBWS_Write14SegNumberHex(uint32 number, uint8 position, uint8 digits, uint8 alignment)
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_WriteString14Seg
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LED_Driver_LRBWS_WriteString14Seg(char8 const character[], uint8 position)

{
    ...
    for(currPosition = position; currPosition < LED_Driver_LRBWS_NUM_COM; currPosition++)
    {
        if(currPosition < sizeLimit)
        {
            charDisplay = LED_Driver_LRBWS_EncodeChar14Seg(character[i]);
            LED_Driver_LRBWS_segLBuffer[currPosition] = LO8(charDisplay);
            #if (LED_Driver_LRBWS_NUM_SEG > 8u)
            LED_Driver_LRBWS_segMBuffer[currPosition] = HI8(charDisplay);
            #endif /* (LED_Driver_LRBWS_NUM_SEG > 8u) */
            i++;
        }
    }
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_PutChar14Seg
********************************************************************************
*
* Summary:
*  Displays an ascii encoded character at 'position'. This function can display
...
*
*******************************************************************************/
void LED_Driver_LRBWS_PutChar14Seg(char8 character , uint8 position)

{
    ...
    if(position < LED_Driver_LRBWS_NUM_COM)
    {
        charDisplay = LED_Driver_LRBWS_EncodeChar14Seg(character);
        LED_Driver_LRBWS_segLBuffer[position] = LO8(charDisplay);
    #if (LED_Driver_LRBWS_NUM_SEG > 8u)
        LED_Driver_LRBWS_segMBuffer[position] = HI8(charDisplay);
    #endif /* (LED_Driver_LRBWS_NUM_SEG > 8u) */
    }
}
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_Write14SegDigitDec
********************************************************************************
*
* Summary:
*  Displays a single digit on the specified display. The number in 'digit' 
...
*
*******************************************************************************/
void LED_Driver_LRBWS_Write14SegDigitDec(uint8 digit, uint8 position)

{
    uint16 charDisplay;
    
    if((digit < 10u) && (position < LED_Driver_LRBWS_NUM_COM))
    {
        charDisplay = LED_Driver_LRBWS_EncodeNumber14Seg(digit);
        LED_Driver_LRBWS_segLBuffer[position] = LO8(charDisplay);
    #if (LED_Driver_LRBWS_NUM_SEG > 8u)
        LED_Driver_LRBWS_segMBuffer[position] = HI8(charDisplay);
    #endif /* (LED_Driver_LRBWS_NUM_SEG > 8u) */
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_Write14SegDigitHex
********************************************************************************
*
* Summary:
*  Displays a single digit at the specified position. The number in 'digit' 
...
*
*******************************************************************************/
void LED_Driver_LRBWS_Write14SegDigitHex(uint8 digit, uint8 position)
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_Write16SegNumberDec
********************************************************************************
*
* Summary:
*  Displays a signed integer up to 8 characters long, starting at 'position' 
...
*
*******************************************************************************/
void LED_Driver_LRBWS_Write16SegNumberDec(int32 number, uint8 position, uint8 digits, uint8 alignment)

{
    ...
    {
        selectVal = (uint8)(numberVal % 10u);
        bufArray[i-1u] = LED_Driver_LRBWS_EncodeNumber16Seg(selectVal);
        numberVal = numberVal / 10u;
    }
    ...
    if(alignment == LED_Driver_LRBWS_ZERO_PAD)
    {
        ...
        {
            /* Place sign on the MSB */
            bufArray[0u] = LED_Driver_LRBWS_EncodeChar16Seg('-');
        }
    }
    else
    {
        for(i = 0u; i < digits; i++)
        {
            if (bufArray[i] != LED_Driver_LRBWS_ZERO_16CHAR)
            {
                if(numberSign == LED_Driver_LRBWS_SIGNED)
                {
                    if(i == 0u)
                    {
                        /* Place sign on the MSB */
                        bufArray[i] = LED_Driver_LRBWS_EncodeChar16Seg('-');
                    }
                    else
                    {
                        /* Place sign before MSB */
                        bufArray[i-1u] = LED_Driver_LRBWS_EncodeChar16Seg('-');
                    }
                }
                /* Exit when encountering non-zero number */
                break;
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_Write16SegNumberHex
********************************************************************************
*
* Summary:
*  Displays a hexadecimal number up to 8 characters long, starting at 'position'
...
*
*******************************************************************************/
void LED_Driver_LRBWS_Write16SegNumberHex(uint32 number, uint8 position, uint8 digits, uint8 alignment)
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_WriteString16Seg
********************************************************************************
*
* Summary:
*  Displays a null terminated string starting at 'position' and ending at either
...
*
*******************************************************************************/
void LED_Driver_LRBWS_WriteString16Seg(char8 const character[], uint8 position)

{
    ...
    for(currPosition = position; currPosition < LED_Driver_LRBWS_NUM_COM; currPosition++)
    {
        if(currPosition < sizeLimit)
        {
            charDisplay = LED_Driver_LRBWS_EncodeChar16Seg(character[i]);
            LED_Driver_LRBWS_segLBuffer[currPosition] = LO8(charDisplay);
            #if (LED_Driver_LRBWS_NUM_SEG > 8u)
                LED_Driver_LRBWS_segMBuffer[currPosition] = HI8(charDisplay);
            #endif /* (LED_Driver_LRBWS_NUM_SEG > 8u) */
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_PutChar16Seg
********************************************************************************
*
* Summary:
*  Displays an ascii encoded character at 'position'. This function can display 
...
*
*******************************************************************************/
void LED_Driver_LRBWS_PutChar16Seg(char8 character , uint8 position)

{
    ...
    if(position < LED_Driver_LRBWS_NUM_COM)
    {
        charDisplay = LED_Driver_LRBWS_EncodeChar16Seg(character);
        LED_Driver_LRBWS_segLBuffer[position] = LO8(charDisplay);
        #if (LED_Driver_LRBWS_NUM_SEG > 8u)
            LED_Driver_LRBWS_segMBuffer[position] = HI8(charDisplay);
        #endif /* (LED_Driver_LRBWS_NUM_SEG > 8u) */
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_Write16SegDigitDec
********************************************************************************
*
* Summary:
*  Displays a single digit on the specified display. The number in 'digit' 
...
*
*******************************************************************************/
void LED_Driver_LRBWS_Write16SegDigitDec(uint8 digit, uint8 position)

{
    ...
    if((digit < 10u) && (position < LED_Driver_LRBWS_NUM_COM))
    {
        charDisplay = LED_Driver_LRBWS_EncodeNumber16Seg(digit);
        LED_Driver_LRBWS_segLBuffer[position] = LO8(charDisplay);
        #if (LED_Driver_LRBWS_NUM_SEG > 8u)
            LED_Driver_LRBWS_segMBuffer[position] = HI8(charDisplay);
        #endif /* (LED_Driver_LRBWS_NUM_SEG > 8u) */
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_Write16SegDigitHex
********************************************************************************
*
* Summary:
*  Displays a single digit at the specified position. The number in 'digit' 
...
*
*******************************************************************************/
void LED_Driver_LRBWS_Write16SegDigitHex(uint8 digit, uint8 position)
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_PutDecimalPoint
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LED_Driver_LRBWS_PutDecimalPoint(uint8 dp, uint8 position)

{
    uint8 value;
    uint8 relPosition;
    
    value = LED_Driver_LRBWS_DEC_POINT_VALUE;
    ...
    if(position < LED_Driver_LRBWS_NUM_COM)
    {
        ...
        {
            #if(LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH)
                LED_Driver_LRBWS_segLBuffer[relPosition] &= ((uint8)(~(uint8)value));
            #else
                LED_Driver_LRBWS_segLBuffer[relPosition] |= ((uint8)(~(uint8)value));
            #endif /* (LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH) */
        }
    ...
    else if (position < LED_Driver_LRBWS_TWO_NUM_COM)
    {
            ...
            {
                #if(LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH)
                    LED_Driver_LRBWS_segMBuffer[relPosition] &= ((uint8)(~(uint8)value));
                #else
                    LED_Driver_LRBWS_segMBuffer[relPosition] |= ((uint8)(~(uint8)value));
                #endif /* (LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH) */
            }
    ...
    else
    {
            ...
            if(relPosition < LED_Driver_LRBWS_NUM_COM)
            {
                ...
                {
                    #if(LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH)
                        LED_Driver_LRBWS_segHBuffer[relPosition] &= ((uint8)(~(uint8)value));
                    #else
                        LED_Driver_LRBWS_segHBuffer[relPosition] |= ((uint8)(~(uint8)value));
                    #endif /* (LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH) */
                }
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_GetDecimalPoint
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 LED_Driver_LRBWS_GetDecimalPoint(uint8 position) 
{
    uint8 value;
    uint8 relPosition;
    ...
    {
        relPosition = position;
        decPointVal = (uint8)((uint8)(LED_Driver_LRBWS_segLBuffer[relPosition]) >> LED_Driver_LRBWS_DEC_POINT);
    } 
    else if (position < LED_Driver_LRBWS_TWO_NUM_COM)
    {
        #if (LED_Driver_LRBWS_NUM_SEG > 8u) 
            relPosition = position - LED_Driver_LRBWS_NUM_COM;
            decPointVal = (uint8)((uint8)(LED_Driver_LRBWS_segMBuffer[relPosition]) >> LED_Driver_LRBWS_DEC_POINT);
        #endif /* (LED_Driver_LRBWS_NUM_SEG > 8u) */
    }
    else
    {
            ...
            if(relPosition < LED_Driver_LRBWS_NUM_COM)
            {
                decPointVal = (uint8)((uint8)(LED_Driver_LRBWS_segHBuffer[relPosition]) >> LED_Driver_LRBWS_DEC_POINT);
            }
        #endif /* (LED_Driver_LRBWS_NUM_SEG > 16u) */
    }
    
    #if(LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH)
        value = (uint8)(decPointVal & LED_Driver_LRBWS_DEC_POINT_MASK);
    #else
        value = (uint8)((uint8)((uint8)~decPointVal) & LED_Driver_LRBWS_DEC_POINT_MASK);
    #endif /* (LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH) */
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_EncodeNumber7Seg
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 LED_Driver_LRBWS_EncodeNumber7Seg(uint8 number) 
{
    /* 7-segment numerical LUT */
    #if(LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH)
        static const uint8 CYCODE LED_Driver_LRBWS_7SegDigits[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
    ...
    #endif /*  LED_Driver_LRBWS_SEG_DRIVE */

    uint8 valNum;
    uint8 valHex;
    
    valNum = (uint8)(number & LED_Driver_LRBWS_NIBBLE_MASK);
    
    valHex = LED_Driver_LRBWS_7SegDigits[valNum];
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_EncodeChar7Seg
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 LED_Driver_LRBWS_EncodeChar7Seg(char8 input)  
{
    /* 7-segment ASCII LUT */
    #if(LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH)
        static const uint8 CYCODE LED_Driver_LRBWS_7SegASCII[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
    ...
    valLow = (uint8)((uint8)input & LED_Driver_LRBWS_NIBBLE_MASK);
    
    /* valHigh used to access the row of LUT */
    valHigh = (uint8)((uint8)input >> LED_Driver_LRBWS_SHIFT_NIBBLE);
    
    /* Check if the high nibble is within valid ASCII range */
    if ((valHigh > LED_Driver_LRBWS_SEG_ASCII_LIM_LO) && (valHigh < LED_Driver_LRBWS_SEG_ASCII_LIM_HI))
    {
        valHigh = valHigh - LED_Driver_LRBWS_SEG_ASCII_OFFSET;
        valHex = LED_Driver_LRBWS_7SegASCII[valHigh][valLow];
    }
    else
    {
        valHex = LED_Driver_LRBWS_CLEAR;
    }
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_EncodeNumber14Seg
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 LED_Driver_LRBWS_EncodeNumber14Seg(uint8 number) 
{
    #if(LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH)
        /* 14-segment numerical LUT for low byte */
        static const uint8 CYCODE LED_Driver_LRBWS_14SegDigitsLow[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
        ...
        static const uint8 CYCODE LED_Driver_LRBWS_14SegDigitsHigh[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
    ...
    valHigh = (uint16)((uint16)LED_Driver_LRBWS_14SegDigitsHigh[valNum] << LED_Driver_LRBWS_SHIFT_BYTE);
    valLow = (uint16)LED_Driver_LRBWS_14SegDigitsLow[valNum];
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_EncodeChar14Seg
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 LED_Driver_LRBWS_EncodeChar14Seg(char8 input) 
{
    #if(LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH)
        /* 14-segment ASCII LUT for low byte */
        static const uint8 CYCODE LED_Driver_LRBWS_14SegASCIILow[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
        ...
        static const uint8 CYCODE LED_Driver_LRBWS_14SegASCIIHigh[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
    ...
    if ((valHigh > LED_Driver_LRBWS_SEG_ASCII_LIM_LO) && (valHigh < LED_Driver_LRBWS_SEG_ASCII_LIM_HI))
    {
        ...
        /* Get the 14-seg hex value from high and low byte LUTs */
        valHexHigh = (uint16)((uint16)LED_Driver_LRBWS_14SegASCIIHigh[valHigh][valLow] << LED_Driver_LRBWS_SHIFT_BYTE);
        valHexLow = (uint16)LED_Driver_LRBWS_14SegASCIILow[valHigh][valLow];
        
        valHex = valHexHigh | valHexLow;
    }
    else
    {
        valHex = LED_Driver_LRBWS_CLEAR_16;
    }
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_EncodeNumber16Seg
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 LED_Driver_LRBWS_EncodeNumber16Seg(uint8 number) 
{
    #if(LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH)
        /* 16-segment numerical LUT for low byte */
        static const uint8 CYCODE LED_Driver_LRBWS_16SegDigitsLow[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
        ...
        static const uint8 CYCODE LED_Driver_LRBWS_16SegDigitsHigh[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
    ...
    valHigh = (uint16)((uint16)LED_Driver_LRBWS_16SegDigitsHigh[valNum] << LED_Driver_LRBWS_SHIFT_BYTE);
    valLow = (uint16)LED_Driver_LRBWS_16SegDigitsLow[valNum];
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_EncodeChar16Seg
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 LED_Driver_LRBWS_EncodeChar16Seg(char8 input) 
{
    #if(LED_Driver_LRBWS_SEG_DRIVE == LED_Driver_LRBWS_HIGH)
        /* 16-segment ASCII LUT for low byte */
        static const uint8 CYCODE LED_Driver_LRBWS_16SegASCIILow[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
        ...
        static const uint8 CYCODE LED_Driver_LRBWS_16SegASCIIHigh[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
    ...
    if ((valHigh > LED_Driver_LRBWS_SEG_ASCII_LIM_LO) && (valHigh < LED_Driver_LRBWS_SEG_ASCII_LIM_HI))
    {
        ...
        /* Get the 16-seg hex value from high and low byte LUTs */
        valHexHigh = (uint16)((uint16)LED_Driver_LRBWS_16SegASCIIHigh[valHigh][valLow] << LED_Driver_LRBWS_SHIFT_BYTE);
        valHexLow = (uint16)LED_Driver_LRBWS_16SegASCIILow[valHigh][valLow];
...
/*******************************************************************************
* Function Name: LED_Driver_LRBWS_SetBrightness
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LED_Driver_LRBWS_SetBrightness(uint8 bright, uint8 position)

{   
    #if(LED_Driver_LRBWS_COM_DRIVE == LED_Driver_LRBWS_HIGH)
        static const uint8 CYCODE LED_Driver_LRBWS_tempCom[16] = {0x00u, 0x01u, 0x00u, 0x02u, 0x00u, 0x04u,
                                                                  0x00u, 0x08u, 0x00u, 0x10u, 0x00u, 0x20u,
                                                                  0x00u, 0x40u, 0x00u, 0x80u};
    #else
        static const uint8 CYCODE LED_Driver_LRBWS_tempCom[16] = {0xFFu, 0xFEu, 0xFFu, 0xFDu, 0xFFu, 0xFBu,
    ...
    if(bright == 0u)
    {
        ...
        LED_Driver_LRBWS_comBuffer[(uint8)((uint8)position << 1u) + 1u] = LED_Driver_LRBWS_CLEAR;
    }
    else
    {
        /* Restore the commons */
        LED_Driver_LRBWS_comBuffer[(uint8)((uint8)position << 1u) + 1u]
            = LED_Driver_LRBWS_tempCom[(uint8)((uint8)position << 1u) + 1u];
    }
    
    /* Update the brightness register */
    LED_Driver_LRBWS_brightnessReg[position] = bright;
}


/*******************************************************************************
* Function Name: LED_Driver_LRBWS_GetBrightness
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 LED_Driver_LRBWS_GetBrightness(uint8 position) 
{
    return(LED_Driver_LRBWS_brightnessReg[position]);
}
